...
namespace addon\membercancel\admin\controller;
 ...
 * 会员注销管理 控制器
 */
class Membercancel extends BaseAdmin
{
    ...
    public function cancelAgreement()
    {
        if (request()->isAjax()) {
            ...
            return $config_model->setCancelDocument($title, $content);
        } else {
            //获取注销协议
            $config_model = new ConfigModel();
    ...
    public function cancelConfig()
    {
        ...
        if (request()->isAjax()) {
            ...
            return $config_model->setCancelConfig($data);
        } else {
            //获取注册设置
