...
typeHtml +=			 '<template v-for="(item,index) in list" v-bind:k="index">';
typeHtml +=		 		'<div class="search_type_left '+post+'" v-on:click="parent[data.field]=item.value,parent.searchStyle=1" :class="{\'active\':parent[data.field]==item.value}">';
...
Vue.component("search-type", {
	...
	created: function () {
		...
		var searchItem = {
			label: "样式3",
			value: 3,
			icon_img:searchResourcePath + "/search/img/search3.png",
			icon_img_active:searchResourcePath + "/search/img/search3_hover"+url+".png",
		};
