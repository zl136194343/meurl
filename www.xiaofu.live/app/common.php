...
*$list         数据列表
*/
function exportCsv($tableHead, $tempLine, $list, $file_name = '')
{
    $array = implode(',', $tableHead);
    $html = $array . "\n";
    $tempLine = implode(',', $tempLine) . "\n";
...
{
    $config = new \app\model\web\Config();
    $info = $config->getPcDomainName();
    $pc_name = $info[ 'data' ][ 'value' ][ 'domain_name_pc' ];
    if ($pc_name) {
        return $pc_name;
    }
    return __ROOT__ . '/web';
            ...
            if ($dur < 3600) {
                ...
                if ($dur < 86400) {
                    ...
                    if ($dur < 259200) {//3天内
                        ...
                        $dur_m = date('m',$the_time);
                        $dur_mt = date('m',time());
                        $dur = date('Y',$the_time);
                        $Y_time = date('Y',time());
                        if ($Y_time !== $dur ) {//3天内

                            return ($Y_time - $dur). '年前';

                        }else{
                            return ($dur_mt - $dur_m) .'月前';
                        }
                        return floor($dur / 86400) . '天前';
