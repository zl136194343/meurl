 ...
 * 版本 1.0.1
 */
class Upgrade extends BaseAdmin
{
    /**
     * 系统授权信息
     */
    ...
    public function executeFile()
    {
        ...
        try {
            ...
            $temp_path =  "upload/upgrade/{$upgrade_no}/public";
            // 先解压客户端文件
            $client_type_arr = [ 'h5', 'web' ];
            $file_manage = new FileManage();
            foreach ($client_type_arr as $client_type) {
                $client_path = $temp_path . '/' . $client_type . '.zip';
    ...
    public function refresh()
    {
        if (request()->isAjax()) {
            ...
            try {
                ...
                $arr = [ '', 'bargain', 'groupbuy', 'pintuan', 'seckill', 'coupon', 'fenxiao', 'live', 'notes', 'store', 'platformcoupon', 'wholesale', 'gift', 'cards', 'egg', 'turntable' ];
                $addon = new Addon();
                foreach ($arr as $k => $v) {
