...
class Message extends BaseModel
{
    ...
    public function getMessageInfo($condition, $field = "*")
    {
        ...
        $info[ "sms_json_array" ] = event('SmsTemplateInfo', [ 'keywords' => $keywords ], true);//短信配置
    ...
    public function getMessageList($condition = [], $field = true, $order = '', $limit = null)
    {
        ...
        if(!empty($list)){
            foreach($list as $k => $v){
                $list[$k]['support_type_array'] = explode(',', $v['support_type']);
        }
        }
    ...
    public function sendMessage($param)
    {
        try {
            ...
            return $this->error('', "MESSAGE_FAIL：" . $e->getMessage() . '，File：' . $e->getFile() . '，Line：' . $e->getLine());
        }
    }
}