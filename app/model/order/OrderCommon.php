...
class OrderCommon extends BaseModel
{
    ...
    public function orderOnlinePay($data)
    {
        ...
        try {
            ...
            //file_put_contents('datess.txt',json_encode($data));
            ...
            foreach ($order_list as $k => $order) {
               ...
               /* $message_model = new Message();
                ...
                /*file_put_contents('datad.txt',json_encode($order));*/
                $res = $message_model->getSendmsg($list['ORDER_ZF'],['character_string1'=>["value"=> $order['order_no']],'date2'=>["value"=> date('Y年m月d日 H:s',time())],'amount3'=>["value"=> $order['pay_money']],'thing4'=>["value"=> $order['order_name']],'number8'=>["value"=> $order['goods_num']]],$order[ 'member_id' ]);
            }
...
//            $invoice_money = $new_goods_money * $order_info['invoice_rate']/100;
     ...
     * @return array
     */
    public function orderSetGh($condition, $uid = 0)
    {
        $order_info = model('order')->getInfo($condition);
        ...
        try {
            ...
            //改变订单状态
            $order_data = array (
                'order_status' => $order_model::ORDER_GH,
                'order_status_name' => $order_model->order_status[ $order_model::ORDER_GH ][ "name" ],
                'order_status_action' => json_encode($order_model->order_status[ $order_model::ORDER_GH ], JSON_UNESCAPED_UNICODE),
/*                "is_evaluate" => 1,
                "evaluate_status" => 0,
                "evaluate_status_name" => "待评价",*/
                "return_time" => time()
    ...
    public function orderSetZl($condition, $uid = 0)
    {
        $order_info = model('order')->getInfo($condition);
        ...
        try {
            ...
            //改变订单状态
            $order_data = array (
                'order_status' => $order_model::ORDER_SYZ,
                'order_status_name' => $order_model->order_status[ $order_model::ORDER_SYZ ][ "name" ],
                'order_status_action' => json_encode($order_model->order_status[ $order_model::ORDER_SYZ ], JSON_UNESCAPED_UNICODE),
                /*"is_evaluate" => 1,*/
                /*"evaluate_status" => 0,
                "evaluate_status_name" => "待归还",*/
                "action_time" => time()
            );
