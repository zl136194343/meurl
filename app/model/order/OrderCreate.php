...
class OrderCreate extends BaseModel
{
    ...
    public function create($data)
    {
        ...
        if ($this->is_exist_free) {
            $free_out_trade_no = $pay->createOutTradeNo($data['member_id']);
            $free_order_name = [];
        }
        ...
        try {
            ...
            foreach ($calculate_data['shop_goods_list'] as $k => $v) {
                ...
                $order_no = $this->createOrderNo($v['site_id'], $data['member_id']);
                ...
                if($v['cash_pledge'] > 0){
                    $data_order['earnest_money'] = $v['cash_pledge'];
                    $data_order['is_lease'] = 1;
                }
                        ...
                        foreach ($present_goods_list as $present_goods_k => $present_goods_v) {
                            ...
                            if ($stock_result["code"] >= 0) {
                                ...
                                $data_order_goods = array(
                                    ...
                                    'goods_name' => $present_goods_v['goods_name'],
                                    'sku_spec_format' => $present_goods_v['sku_spec_format'],
                                );
                                model("order_goods")->add($data_order_goods);
...
//                                return $stock_result;
            ...
            foreach ($order_id_arr as $k => $v) {
                ...
                if (!empty($result_list)) {
                    foreach ($result_list as $result_k => $result_v) {
                        if (!empty($result_v) && $result_v["code"] < 0) {
                            model("order")->rollback();
                            return $result_v;
                        }
                    }
     ...
     * @param array $site_id
     */
    public function createOrderNo($site_id, $member_id)
    {
//        $time_str = date('YmdHi');
//        $num = 0;
...
//        Cache::set($site_id . "_" . $time_str, $max_no);
    ...
    public function calculate($data)
    {
       ...
       foreach( $data['shop_goods_list']as &$item){
            //判断当前用户是否有参与会有优惠
             
            $is_member = model('shop')->getInfo([['site_id','=',$item['site_id']]],'is_member');
            ...
            $mem_info  = model('member')->getInfo([['member_id','=',$data['member_id']]],'member_level');
            //查出对应优惠力度\

            $mem_info  = model('member_level')->getInfo([['level_id','=',$mem_info['member_level']]]);
            
                if($mem_info['discount'] > 0){
                    //用户等级优惠
                    $mem_money = round($item['goods_money']*$mem_info['discount'],2)/100 ;
                    $dj = $item['goods_money'] - $mem_money;
                     $this->dj_money += $item['goods_money'] - $mem_money;
                     
                    $item['order_money'] = $item['order_money'] - $dj;
                    $item['dj_money'] = $dj;
                    foreach ($item['goods_list'] as &$value) {
                      
                       $value['real_goods_money'] = $value['real_goods_money']- $dj;
                    }
                    /*$item['real_goods_money'] = $item['order_money'];*/
...
//        }
        ...
        foreach( $data['shop_goods_list']as &$item){
            ...
            foreach ($item['goods_list'] as &$i){
                
                
                if($i['is_lease'] != 0 ){
                    //说明是租赁商品
                    $item['cash_pledge'] =floor($i['cash_pledge'] * $i['num'] * 100 )/100 ;
                     $item['order_money'] += floor($i['cash_pledge'] * $i['num'] * 100 )/100 ;
                    /*$this->order_money +=floor($i['cash_pledge'] * $i['num'] * 100 )/100 ;*/
                    $this->cash_pledge += floor($i['cash_pledge'] * $i['num'] * 100 )/100 ;
                    /*$i['real_goods_money'] = $i['real_goods_money'] + $this->cash_pledge;*/
                   /* $i['pay_money'] += $i['pay_money'] + $this->cash_pledge;*/
                }else{
                    $item['cash_pledge'] = 0;
                }
            }
        ...
        $data['dj_money'] = $this->dj_money;
        $data['cash_pledge'] = $this->cash_pledge;
...
//        $cart_ids, $member_id
        ...
        $field = ' sku_id, sku_name, sku_no, 
            ...
            is_free_shipping, shipping_template, goods_class, goods_class_name, goods_id,max_buy,min_buy,goods_name,sku_spec_format,cash_pledge,is_lease';
        ...
        if (!empty($goods_list)) {
            foreach ($goods_list as $k => $v) {
                //todo  移除sku的佣金比率,取用所属商品的佣金比率
                $goods_info = model('goods')->getInfo([['goods_id', '=', $v['goods_id']]], 'commission_rate,goods_image,wj_off');
            ...
            if (!empty($discount_array['rule'])) {
                ...
                if ($this->is_virtual == 0) {
                    ...
                    if (!empty($present_value)) {
                        ...
                        $present_sku_id = $present_value['sku_id'] ?? 0;
                        $present_num = $present_value['present_num'] ?? 0;

                        $present_goods_list = $present_list['goods_list'] ?? [];
                        $present_goods = [['present_id' => $present_id, 'present_num' => $present_num,'sku_id' => $present_sku_id]];
                ...
                foreach ($manjian_list as $k => $v) {
                    ...
                    if (!empty($discount_array['rule'])) {
                        ...
                        if (!empty($present_value)) {
                            ...
                            $present_sku_id = $present_value['sku_id'] ?? 0;
                            $present_num = $present_value['present_num'] ?? 0;
                            $present_goods_ids[] = ['present_id' => $present_id, 'present_num' => $present_num, 'sku_id' => $present_sku_id];
                        }
                        $present_coupon = $discount_array['rule']['coupon'] ?? 0;
                        if ($present_coupon > 0) {
...
//        $is_self = false;
        ...
        if ($is_self) {
            $self = $this;
            foreach ($shop_goods_list as $k => $v) {
                $shop_goods_list[$k] = $this->eachShopOrderItem($v, $self, $is_self, $data);
            }
        } else {
            $shop_goods_list = $this->eachShopOrderItem($shop_goods_list, $self, $is_self, $data);
        }
     ...
     * @return mixed
     */
    public function eachShopOrderItem($item, $self, $is_self, $data)
    {

        //发票相关
        $item = $this->invoice($item, $data);

        $item['order_money'] = $item['order_money'] + $item['invoice_money'] + $item['invoice_delivery_money'];
        ...
        if($mem_info['discount'] > 0){
            ...
            $item['order_money'] = $item['order_money']*$mem_info['discount'];
        }*/
...
//                    $self->order_money -= $real_platform_coupon_money;
                    ...
                    $temp_shop_goods_list = $self->distributionPlatformCouponMoney($temp_shop_goods_list, $platform_order_money,
...
//            $invoice_content_array = explode(',', $invoice_content);
    ...
    public function presentGoods($data_goods_list, $present_list)
    {
        ...
        if (!empty($data_goods_list)) {
            $present_column_list = array_column($present_list, null, 'sku_id');
            $temp_goods_list = [];
            foreach ($data_goods_list as $data_list_k => $goods_list_v) {
                $item_key = $goods_list_v['present_id'].'_'.$goods_list_v['sku_id'];//聚合同键数据
                $num = $temp_goods_list[$item_key] ?? 0;
                $temp_goods_list[$item_key] = $num + $goods_list_v['present_num'];
            }
            foreach ($temp_goods_list as $goods_k => $goods_item) {
                if ($goods_item <= 0) {
                    continue;
                }
                $item_key_data = explode('_', $goods_k);
                $present_id = $item_key_data[0];
                $present_sku_id = $item_key_data[1];
                $item_condition = array(
                    ['present_id', '=', $present_id],
                    ['status', '=', 2],
                );
                $num = $goods_item;
                $present_info = $present_model->getPresentInfo($item_condition, '*')['data'];
                //赠品失效的情况不需要考虑
                if (!empty($present_info)) {

                    $present_goods_condition = array(
                        ['pg.present_id', '=', $present_id],
                        ['pg.sku_id', '=', $present_sku_id]
                    );
                    $present_goods_info = $present_model->getPresentGoodsDetail($present_goods_condition)['data'] ?? [];
                    if(empty($present_goods_info))
                        continue;


                    //商品状态必须上架 销售中   ,必须未删除
                    if($present_goods_info['goods_state'] != 1 || $present_goods_info['verify_state'] != 1 || $present_goods_info['is_delete'] == 1){
                        continue;
                    }
                    if ($present_goods_info['stock'] <= 0) {
                        continue;
                    }

                    //数量足够的话
                    if ($present_goods_info['stock'] < $num) {
                        $num = $present_goods_info['stock'];
                    }
                    if ($goods_item <= 0) {
                        continue;
                    }
                    $column_list_item = $present_column_list[$present_goods_info['sku_id']] ?? [];
                    if (!empty($column_list_item)) {
                        $column_list_item['num'] += $num;
                        if ($present_goods_info['stock'] < $column_list_item['num']) {
                            $column_list_item['num'] = $present_goods_info['stock'];
                        }
                        if ($column_list_item['num'] <= 0) {
                            continue;
                        }
                        $present_column_list[$present_goods_info['sku_id']] = $column_list_item;
                    } else {
                        $shop_goods_data = array(
                            'sku_id' => $present_goods_info['sku_id'],
                            'num' => $num,
                            'is_present' => 1,
                            'present_id' => $present_id
                        );
...
//                ['count-lead_count', '>', 0]
    ...
    public function getShopPresentGoodsInfo($data)
    {
        $sku_info = model("goods_sku")->getInfo([['sku_id', '=', $data['sku_id']]], 'sku_id, sku_name, sku_no, price, discount_price,
             cost_price, stock, volume, weight, sku_image, site_id, site_name,
             website_id, is_own, goods_state, is_virtual, is_free_shipping, shipping_template,goods_class, goods_class_name, goods_id, goods_name, sku_spec_format');

        $goods_info = model('goods')->getInfo([['goods_id', '=', $sku_info['goods_id']]], 'commission_rate,goods_image');
        $sku_info['commission_rate'] = $goods_info['commission_rate'] ?? 0;
        $sku_info['goods_image'] = $goods_info['goods_image'] ?? '';
     ...
     * @param $order_id
     */
    public function orderCreateAfter($order_id){
        //订单生成的消息
