...
class VirtualOrder extends OrderCommon
{
    ...
    public function orderPay($order_info, $pay_type)
    {
        ...
        $data = array (
            ...
            'order_status' => 3,
            'order_status_name' => '待核销',
            'order_status_action' => json_encode([
                'status' => 3,
                'name' => '待核销',
                'is_allow_refund' => 1,
                'icon' => 'upload/uniapp/order/order-icon-close.png',
        ...
        if ($order_info[ 'is_lock' ] == 0) {
            ...
            $this->orderTakeDelivery2($order_info[ 'order_id' ]);
        }
     ...
     */
    public function orderTakeDelivery3($order_id)
    {
        ...
        try {
            //订单项变为已发货
            model('order_goods')->update([ 'delivery_status' => 1, "delivery_status_name" => "已收货" ], [ [ 'order_id', "=", $order_id ] ]);
            ...
            $code_result = $verify_model->addVerify("virtualgoods", $order_info[ 'site_id' ], $order_info[ 'site_name' ], $verify_content_json, $order_id);
            $code = $code_result[ "data" ][ "verify_code" ];
            $result = $verify_model->qrcode($code, "all", "virtualgoods", "create");//生成二维码
            //自动收发货
            $order_data = array (
                'virtual_code' => $code,
            );
    ...
    public function orderTakeDelivery($order_id)
    {
        ...
        try {
            //订单项变为已发货
            $res =model('order_goods')->update([ 'delivery_status' => 1, "delivery_status_name" => "已收货" ], [ [ 'order_id', "=", $order_id ] ]);
            // $order_goods_info = model('order_goods')->getInfo([ [ 'order_id', '=', $order_id ] ], 'sku_id,sku_name,sku_image,price,num,order_goods_id');//订单项详情
            // //创建待核销记录
            // $verify_model = new VerifyModel();
            // $item_array = array (
    ...
    public function orderTakeDelivery2($order_id)
    {
        $order_info = model("order")->getInfo([['order_id', '=', $order_id]], 'order_no,site_id,site_name,member_id,order_type,
        sign_time,order_status,delivery_code,create_time,name,pay_time,pay_money,mobile,is_lock,order_money');
        if (empty($order_info))
