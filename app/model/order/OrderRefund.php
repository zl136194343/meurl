...
//            $order_info = model('order')->getInfo(['order_id' => $order_goods_info['order_id']]);
    ...
    public function orderRefundConfirm($data, $user_info = [])
    {
        ...
        try {
            ...
            if($data['money'] !== $data['hidden_money']){
                //说明商户是部分退款
                $data['refund_real_money'] = $data['money'];
                $data['refund_apply_money'] = $data['money'];
            }
            unset($data['money']);unset($data['hidden_money']);
            $res = model('order_goods')->update($data, [ 'order_goods_id' => $data[ 'order_goods_id' ] ]);
     ...
     * @param array $user_info
     */
    public function orderRefundZl($data, $user_info = [])
    {
        $condition = array (
            [ 'order_id', '=', $data[ 'order_goods_id' ] ],
            [ 'site_id', '=', $data[ 'site_id' ] ]
        );
        ...
        if ($order_info[ 'is_lease' ] != 1 && $order_info[ 'is_th_money' ] == 1) {
            return $this->error([], '当前租赁订单已完成退款,请勿重复操作');
        }
...
/*            if ($order_goods_info[ 'refund_type' ] == 1) {
            ...
            $data[ 'refund_status' ] = self::REFUND_ZL;
            $data[ 'refund_status_name' ] = $this->order_refund_status[ $data[ 'refund_status' ] ][ "name" ];
            $pay_model = new Pay();
            $data[ 'refund_no' ] = $pay_model->createRefundNo($data[ 'order_goods_id' ]);
            $data[ 'refund_status_action' ] = json_encode($this->order_refund_status[ $data[ 'refund_status' ] ], JSON_UNESCAPED_UNICODE);
            if($data['money'] !== $data['hidden_money']){
                ...
                $data['refund_real_money'] = $data['hidden_money'];
                $data['refund_apply_money'] = $data['hidden_money'];
            }
            
            unset($data['money']);unset($data['hidden_money']);
            $data['order_goods_id'] = $order_goods_info['order_goods_id'];
            
            /*dump($data);die;*/
            $res = model('order_goods')->update($data, [ 'order_goods_id' => $order_goods_info[ 'order_goods_id' ] ]);
            ...
            $datas = array (
             ...
             $res = $this->orderRefundFinish2($datas,$user_info);
            
            //卖家退还押金消息
            // $message_model = new Message();
            /*$message_model->sendMessage([ 'keywords' => "ORDER_REFUND_AGREE", 'order_id' => $order_goods_info[ 'order_id' ], "order_goods_id" => $data[ 'order_goods_id' ] ]);*/
            
            $res = model('order')->update(['is_th_money'=>1], [ 'order_id' => $order_goods_info[ 'order_id' ] ]);
            $this->addOrderRefundLog($order_goods_info[ 'order_goods_id' ], $data[ 'refund_status' ], '卖家退还押金', 2, $user_info[ 'uid' ] ?? 0, $user_info[ 'username' ] ?? '');
            
            $message_model = new WeappModel();
                $list = $message_model ->lidsList();
                
                $message_model->getSendmsg($list['MEMBER_TK'],['character_string1'=>["value"=> $order_info['order_no']],'amount2'=>["value"=>  $data['refund_real_money']],'time3'=>["value"=> date('Y-m-d',time())],'thing4'=>["value"=> '卖家退还押金'],'thing5'=>["value"=> $user_info[ 'username' ]]],$order_goods_info[ 'member_id' ]);
...
//            $data["refund_reason"] = "";
    ...
    public function orderRefundFinish($data, $user_info)
    {
        ...
        $order_goods_info = model('order_goods')->getInfo($condition);
        $order_info = model('order')->getInfo([[ 'order_id', '=', $order_goods_info[ 'order_id' ] ],[ 'site_id', '=', $data[ 'site_id' ]]]);
        if (empty($order_goods_info)) {
            return $this->error([], 'ORDER_GOODS_EMPTY');
        }
        ...
        try {
            ...
            if ($order_goods_info['refund_apply_money'] != $order_goods_info['real_goods_money']  && $order_info['is_lease'] != 1){
                //说明是部分退款,重新计算对应的店铺与平台以及绑定店铺的实际金额
                $money = $order_goods_info['real_goods_money'] - $order_goods_info['refund_apply_money'];
                $update_data['platform_money'] = round($order_goods_info['commission_rate']*$money/100,2);
                $update_data['shop_money'] = $money-$update_data['platform_money'];
                //获取指定的商户佣金倍率
                $config = new config();
                $res = $config ->getConfig([ [ 'site_id', '=', 0 ], [ 'app_module', '=', 'admin' ], [ 'config_key', '=', 'CHECK_RATE_CONFIG' ] ]);
                $update_data['hospital_money'] = round($res['data']['value']['check_rate']*$update_data['platform_money']/100,2);
                $update_data['platform_money'] = $update_data['platform_money'] - $update_data['hospital_money'];
            }
...
//            event('OrderRefundFinish', $order_goods_info);//传入订单类型以及订单项id
     ...
     * @return multitype:string mixed
     */
    public function orderRefundFinish2($data, $user_info)
    {
        ...
        /*dump($order_goods_info[ 'refund_status' ] != self::REFUND_TAKEDELIVERY && $order_goods_info[ 'refund_status' ] != self::REFUND_CONFIRM);die;*/
        if ($order_goods_info[ 'refund_status' ] != self::REFUND_ZL ) {
            return $this->error();
        }
        model('order_goods')->startTrans();
        try {
            ...
            //退货日志
            $this->addOrderRefundLog($data[ 'order_goods_id' ], self::REFUND_COMPLETE, '押金已退还', 2, $user_info[ 'uid' ], $user_info[ 'username' ]);
...
//                }
            ...
            //验证订单锁定状态
            $this->verifyOrderLock($order_goods_info[ "order_id" ]);
            //验证订单是否全部退款完毕  订单如果全部退款完毕,订单关闭,租赁订单不关闭
            if ($order_goods_count == $refund_count && $order_info['is_lease'] != 1) {
