...
//        }
    ...
    public function calculate($shop_goods, $data)
    {
        ...
        if ($local_info[ 'time_is_open' ] == 1) {
            ...
            if (in_array($week, $local_info[ 'time_week' ])) {
                ...
                if ($time == 0) {
                    ...
                    if ($time < $local_info[ 'start_time' ] || $time > $local_info[ 'end_time' ]) {
                        $time_error++;
                        $error = '配送时间不在营业时间内';
                    }
                }
        ...
        $shop_model = new Shop();
        $shop_info_result = $shop_model->getShopInfo([ [ 'site_id', '=', $shop_goods[ 'site_id' ] ] ]);
        ...
        if ($local_info[ 'area_type' ] == 1 || $local_info[ 'area_type' ] == 2) {
            ...
            if ($local_info[ 'area_type' ] == 1) {
                ...
                if ($distance <= $local_info[ 'start_distance' ]) {//是否在起送距离以内
                    ...
                    if ($local_info[ 'continued_distance' ] == 0) {
                        return $this->error([ 'code' => $error_code ], '当前外卖配送没有配置完善,请联系店铺管理员');
                    }
