...
class GoodsImport extends BaseModel
{
    ...
    public function importGoods($data, $table_head, $table_line, $site_id)
    {
        try {
            ...
            foreach ($data as $k => $v) {
                ...
                if (!isset($temp_list[$v['goods_id']])) {
                    ...
                    $item_temp = array(
                        ...
                        'goods_attr_name' => '',
                        'goods_attr_format' => '',
                        'goods_attr_array' => $v['goods_attr_name'],
                        'goods_state' => 0,
            ...
            $path = "common/import_goods/images/" . date("Ymd") . '/';
            foreach ($temp_list as $k => $v) {
                ...
                if (!empty($v['category_list'])) {
                    ...
                    $item_goods_category_array = $v['category_list'];
                    ...
                    if (!empty($item_category)) {
                        ...
                        //分类
                        $temp_category_item_array = [];
                        if($category_result['category_id_1'] > 0){
                            $temp_category_item_array[] = $category_result['category_id_1'];
                        }
                        if($category_result['category_id_2'] > 0){
                            $temp_category_item_array[] = $category_result['category_id_2'];
                        }
                        if($category_result['category_id_3'] > 0){
                            $temp_category_item_array[] = $category_result['category_id_3'];
                        }
                        $temp_list[$k]['category_array'][0] = implode(',', $temp_category_item_array);
                        $temp_list[$k]['category_json'] = json_encode($temp_list[$k]['category_array']);
                        $temp_list[$k]['category_id'] = ','.implode(',', $temp_list[$k]['category_array']).',';
...
//                        $temp_list[$k]['category_name'] = $category_result['category_full_name'] ?? '';
                        ...
                        //整理属性
                        $item_goods_attr_array = $v['goods_attr_array'] ?? [];
                        if(!empty($item_goods_attr_array)){
                            $goods_attr_array = [];
                            $item_goods_attr_array = explode('，', $item_goods_attr_array);
                            $goods_attr_array_sort = 0;
                            $second = date('s');//当前的秒
                            $time = time() * 1000;
                            $precise_time = Carbon::now()->getPreciseTimestamp(3);
                            $precise_time = $precise_time - $time;
                            $attr_class_id = -($second + $precise_time);
                            $attr_id = -2*($second + $precise_time);
                            foreach($item_goods_attr_array as $item_goods_attr_array_k => $item_goods_attr_array_v) {
                                $item_item_goods_attr_array_v = explode(':', $item_goods_attr_array_v);
                                $attr_name = $item_item_goods_attr_array_v[0] ?? '';
                                $attr_value_name = $item_item_goods_attr_array_v[1] ?? '';
                                $attr_value_id = -($goods_attr_array_sort + $second + $precise_time);
                                ...
                                $goods_attr_array[] = array(
                                    ...
                                    'sort' => $goods_attr_array_sort,
                                );
                                $goods_attr_array_sort++;
                            }
                            $temp_list[$k]['goods_attr_format'] = json_encode($goods_attr_array, JSON_UNESCAPED_UNICODE);
                        }
                        ...
                        //将图片拉取到本地
                        $item_goods_image = $v['goods_image'];//商品主图


                        $remote_result = $upload_model->setPath($path)->remoteGoodsPullToLocal(['img' => $item_goods_image,"thumb_type" => [ "big", "mid", "small" ]]);
                        if($remote_result['code'] >= 0){

                            $temp_list[$k]['goods_image'] = $remote_result['data']['pic_path'];
                            //用于规格个体的值的翻译
                            $spec_dict = array_column($spec_array, null, 'spec_name');
                            $goods_sku_data = [];
                            //单规格模式
                            $item_sku_image_error = [];
                            foreach ($v['list'] as $spec_list_k => $spec_list_v) {
                                $sku_spec_format = [];
                                $item_goods_image_array = $spec_list_v['goods_image_array'];
                                $new_item_goods_image_array = [];
                                //循环将图片拉取到本地
                                foreach($item_goods_image_array as $item_sku_image_k => $item_sku_image_v){
                                    $item_remote_result = $upload_model->setPath($path)->remoteGoodsPullToLocal(['img' => $item_sku_image_v,"thumb_type" => [ "big", "mid", "small" ]]);
                                    if($item_remote_result['code'] < 0){
                                        $item_sku_image_error['error_sku_key'] = $spec_list_k;
                                        $item_sku_image_error['error_img_path'] = $item_sku_image_v;
                                        //图片拉取失败
                                        break 2;
                                    }else{
                                        $new_item_goods_image_array[] = $item_remote_result['data']['pic_path'];
                                    }
                                }
                                //判断规格是否存在
                                $item_goods_sku_data = [
                                    ...
                                    'stock_alarm' => $spec_list_v['stock_alarm'] ?? $temp_list[$k]['goods_stock_alarm'],
                                    'sku_image' => $new_item_goods_image_array[0],
                                    'sku_images' => implode(',', $new_item_goods_image_array),
                                    'sku_images_arr' => $new_item_goods_image_array,
                                ];
                            ...
                            //如果存在图片拉取失败, 将这个错误保存下来
                            if(empty($item_sku_image_error)){
                                if (count($v['list']) > 1) {
                                ...
                                $item_error = '图片'.$item_sku_image_error['error_img_path'].'无法拉取到当前站点';
                                $error_data[] = '图片'.$item_sku_image_error['error_img_path'].'无法拉取到当前站点';
                                $sku_error_list = $v['list'];
                                foreach ($sku_error_list as $sku_error_k => $sku_error_v) {
                                    if($sku_error_k == $item_sku_image_error['error_sku_key']){
                                        $sku_error_list[$sku_error_k]['error'] = '图片'.$item_sku_image_error['error_img_path'].'无法拉取到当前站点';
                                    }else{
                                        $sku_error_list[$sku_error_k]['error'] = '';
                                    }
                            ...
                            $item_error = '主图'.$item_goods_image.'无法拉取到当前站点';
                            $error_data[] = '主图'.$item_goods_image.'无法拉取到当前站点';
                            $sku_error_list = $v['list'];
                            foreach ($sku_error_list as $sku_error_k => $sku_error_v) {
                                $sku_error_list[$sku_error_k]['error'] = '主图'.$item_goods_image.'无法拉取到当前站点';
                            }
                            $error_sku_list = array_merge($error_sku_list, $sku_error_list);
                        }
                    }else{
