...
class GoodsShopCategory extends BaseModel
{
	 ...
	 * @return \multitype
	 */
	public function getShopCategoryTree($condition = [], $field = 'category_id,category_name,short_name,pid,level,is_show,sort,image,category_id_1,category_id_2', $order = 'sort asc', $limit = null)
	{
     ...
     */
    public function modifyGoodsShopCategorySort($sort, $category_id, $site_id)
    {
        ...
        $res = model('goods_shop_category')->update([ 'sort' => $sort ], [ [ 'category_id', '=', $category_id ], [ 'site_id', '=', $site_id ] ]);
        Cache::tag("goods_shop_category" . $site_id)->clear();
        return $this->success($res);
    }
