...
class Model
{
    ...
    final public function rawPageList($condition = [], $field = true, $order = '', $page = 1, $list_rows = PAGE_LIST_ROWS, $alias = 'a', $join = [], $group = null, $limit = null)
    {
        if (is_array($order)) {
            self::$query_obj = Db::name($this->table)->alias($alias)->where($condition)->order($order);
            $count_obj = Db::name($this->tablegetList)->alias($alias)->where($condition)->order($order);
        } else {
    ...
    {
        return Db::name($this->table)->getLastSql();
    }
     ...
     * @param unknown $where
     */
    final public function getJoinCount($where = [], $field = '*', $alias = 'a', $join = null)
    {
        if (empty($join)) {
            return Db::name($this->table)->where($where)->count($field);
        } else {
     ...
     * 判断是否为视频还是img
     */
    final public function getIsImg($content)
    {
        $houzuo = substr($content, strripos($content, '.')+1);
        if ($houzuo == 'png' || $houzuo == 'jpeg'|| $houzuo == 'jpg') {

            $is_video = 0;
        }else if ($houzuo == '.mp4'|| $houzuo == '.avi'||$houzuo == '.wmv'||$houzuo == '.rm'||$houzuo == '.ram'){
            $is_video = 1;
        }
        return $is_video;
    }
     ...
     * 获取视频封面图
     * */
    public function getVideoCover( $input )
    {
        /*$input = "http://grains.oss-cn-beijing.aliyuncs.com/upload/common/video/20221220/20221220025730167151945098150.mp4";*/
        $one = strripos($input, '/')+1 ;
        $t = strripos($input, '.');
        $num = $t- $one ;

        $houzuo = substr($input,$one, $num);
        $name =__UPLOAD__."/common/video/".$houzuo."_video.jpg";

        $command = "ffmpeg -v 0 -y -i $input -vframes 30 -ss 1 -vcodec mjpeg -f rawvideo  -aspect 16:9  $name";
        shell_exec( $command );
        return $name;
    }
