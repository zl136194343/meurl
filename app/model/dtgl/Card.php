...
class Card extends BaseModel
{
    ...
    {
        $res = model('card')->update($data, $condition);
        if ($res === false) {
            return $this->error('', 'RESULT_ERROR');
        }
        return $this->success($res);
    ...
    {
        $res = model('card')->update([
            'status' => $status
        ], $condition);
        if ($res === false) {
            return $this->error('', 'RESULT_ERROR');
        }
        Cache::set('card_blacklist', null);
        return $this->success($res);
    }*/
    public function modifyCardStatus($status, $condition)
    {
        $res = model('card')->update([
            'is_status' => $status
        ], $condition);
    ...
    {
        $res = model('card')->delete($condition);
        if ($res === false) {
            return $this->error('', 'RESULT_ERROR');
        }
        return $this->success($res);
    ...
    public function getMemberInfo($condition = [], $field = '*')
    {
        ...
        $member_info = model('card')->getInfo($condition, $field);
    ...
    public function getMemberDetail($member_id)
    {
        ...
        $member_info = model('card')->getInfo([ [ 'member_id', '=', $member_id ] ], $field);
    ...
    {
        /*$condition[] = [ 'is_delete', '=', 0 ];*/
        $list = model('card')->pageList($condition, $field, $order, $page, $page_size, '', '', '');
    ...
    {
        $condition[] = [ 'is_delete', '=', 0 ];
        $res = model('card')->getList($where, $field, $order, $alias, $join, $group, $limit);
        return $this->success($res);
    }
...
//            "site_name" => $data[ "site_name" ],//站点名称
    ...
    {
        $info = model('card')->getInfo([ [ 'member_id', '=', $member_id ] ], 'pay_password');
    ...
    {
//        $member_info = model('card')->getCount($condition);
//        return $this->success($member_info);
        $condition[] = [ 'is_delete', '=', 0 ];
    ...
    public function getMemberCountByArea($handle = false)
    {
        ...
        array_push($data,[
            'name' => '未知',
            'value'  => $total_count - $num,
            'ratio' => sprintf("%.2f",($total_count - $num) / $total_count * 100 )
    ...
    {
        $cache = Cache::get('card_blacklist');
        if (!empty($cache)) return $this->success($cache);

        $blacklist = model('card')->getColumn([ [ 'status', '=', 0 ] ], 'member_id');
        Cache::set('card_blacklist', $blacklist);
