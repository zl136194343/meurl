...
//                                $condition[] = [ "", 'exp', Db::raw($temp_sql) ];
    ...
    {
        $list = model('member_cluster')->getList([], 'rule_json,cluster_id');
        //循环刷新事件
    ...
    public function sendPoint($point, $cluster_id, $remark)
    {
        ...
        $member_list = model("member")->getList([ [ "member_id", "in", $member_cluster_info[ "member_ids" ] ] ], 'member_id');
        $member_account_model = new MemberAccount();
        model('member_account')->startTrans();
        try {
            foreach ($member_list as $value) {
                $member_account_model->addMemberAccount($value[ 'member_id' ], 'point', $point, 'adjust', '会员群体发放积分' . $point, $remark);
            }
    ...
    public function sendBalance($balance, $cluster_id, $remark)
    {
        ...
        try {
            foreach ($member_list as $value) {
                $member_account_model->addMemberAccount($value[ 'member_id' ], 'balance', $balance, 'adjust', '会员群体发放红包' . $balance, $remark);
            }
            model('member_account')->commit();
            return $this->success();
        } catch (\Exception $e) {
        ...
        try {
            foreach ($member_list as $value) {
                ...
                foreach ($coupon_array as $k => $v) {
                    $res =$coupon_model->receivePlatformcoupon($v, $value[ 'member_id' ], 3);
                }
            }
