...
use addon\weapp\event\Qrcode as addQrcode;
/**
 * 店铺信息（无缓存）
 */
class Shop extends BaseModel
{
    ...
    public function addShop($shop_data, $cert_data, $user_info)
    {
        ...
        try {
            ...
            /*$site_xgl_id= date('Ymd').str_pad($apply_info[ 'uid' ],3,0,STR_PAD_LEFT);*/
            /*$shop_data['site_xgl_id'] = $site_xgl_id;*/
            ...
            $re = model("user")->add($user_info);
            
            $site_xgl_id= date('Ymd').str_pad($re,3,0,STR_PAD_LEFT);
            $shop_data['site_xgl_id'] = $site_xgl_id;
...
//		客服配置信息
     ...
     * 店铺二维码生成(指定小程序)
     */
    public function  getqrcode($site_id,$type= "create")
    {
        $data = [
            'app_type' => "all", // all为全部
            'type' => $type, // 类型 create创建 get获取
            ...
            'page' => '/otherpages/shop/index/index',
            'qrcode_path' => 'upload/qrcode/shop/',
            'qrcode_name' => "shop_dp_qrcode_" . $site_id,
        ];
        $qrcode = new addQrcode();
        $res = $qrcode->handle($data);
        dump($res);die;
        //获取appid 以及key
    }
