...
class Games extends BaseModel
{
        ...
        try {
            ...
            foreach ($award_json_data as $k => $v) {
                $item = [
                    ...
                    'balance' => isset($v['balance']) ? $v['balance'] : 0,
    ...
    {
        // 获取游戏信息
        $game_info = model('promotion_games')->getInfo([['game_id', '=', $game_id]]);
        if (empty($game_info)) return $this->error("", "未获取到游戏信息");
        $site_id = $game_info['site_id'] ?? 0;
        // 判断游戏状态
        if ($game_info['status'] == 0) return $this->error("", "游戏尚未开始");
        if ($game_info['status'] == 2 || $game_info['status'] == 3) return $this->error("", "游戏已经结束");

        // 获取会员信息
        $member_info = model('member')->getInfo([['member_id', '=', $member_id], ['status', '=', 1]], 'nickname,member_level,point');
        ...
        try {
            ...
            if ($game_info['points'] > 0) {
                // 扣除积分
                $member_account->addMemberAccount($member_id, 'point', -$game_info['points'], $game_info['game_type'], $game_id, "{$game_info['game_type_name']}消耗积分");
            }
            // 抽奖人数
            model('promotion_games')->setInc([['game_id', '=', $game_id]], 'join_num');
            $record = [
                'site_id' => $site_id,
                'game_id' => $game_id,
            ...
            if ($lottery_result['is_winning']) {
                switch ($lottery_result['award_type']) {
                    case 1:
                        // 积分
                        $member_account->addMemberAccount($member_id, 'point', $lottery_result['point'],
                            $game_info['game_type'], $game_id, "{$game_info['game_type_name']}获得积分");
                        break;
                    case 2:
                        // 余额
                        $member_account->addMemberAccount($member_id, 'balance', $lottery_result['balance'],
                            $game_info['game_type'], $game_id, "{$game_info['game_type_name']}获得余额");
                        break;
                    case 3:
                        // 优惠券
                        if ($site_id > 0) {
                            // 店铺优惠券
                            $coupon = new Coupon();
                            $receive_res = $coupon->receiveCoupon($lottery_result['relate_id'], $member_id, '', 0, 0);
                            // 如果优惠券发放失败则本次抽奖为未中奖
                            if ($receive_res['code'] < 0) $lottery_result = ['is_winning' => 0];
                        } else {
                            // 平台优惠券
                            $coupon = new Platformcoupon();
                            $receive_res = $coupon->receivePlatformcoupon($lottery_result['relate_id'], $member_id, '', 0, 0);
                            // 如果优惠券发放失败则本次抽奖为未中奖
                            if ($receive_res['code'] < 0) $lottery_result = ['is_winning' => 0];
                        }
                        break;
     ...
     * @return array
     */
    public function getMemberSurplusNum($game_id, $member_id, $site_id = 0)
    {
        $num = 0;
        $game_condition = [
            ['game_id', '=', $game_id],
        ...
        if ($site_id > 0) {
            $game_condition[] = ['site_id', '=', $site_id];
        }
        $game_info = model('promotion_games')->getInfo($game_condition, 'join_type,join_frequency');
        if (!empty($game_info)) {
            if ($game_info['join_type']) {
                // 每天
                $tody_start = strtotime(date("Y-m-d"), time());
        ...
        foreach ($app_type_list as $k => $v) {
            switch ($k) {
                case 'h5':
                    ...
                    $path[$k]['status'] = 1;
                    $path[$k]['url'] = $h5_domain . $data['page'] . '?id=' . $game_id . '&site_id=' . $site_id;
