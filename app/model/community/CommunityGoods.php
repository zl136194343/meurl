...
use app\model\community\GoodsCategory;
use Carbon\Carbon;

/**
 * 商品
 */
class CommunityGoods extends BaseModel
{
    ...
    {
        model('community_goods')->startTrans();
        ...
        try {
            ...
            if (empty($goods_category_info)) {
                model('community_goods')->rollback();
...
//
            ...
            $goods_id = model('community_goods')->add(array_merge($goods_data, $common_data));
            ...
            model('community_goods_sku')->addList($sku_arr);

            // 赋值第一个商品sku_id
            $first_info = model('community_goods_sku')->getFirstData([ 'goods_id' => $goods_id ], 'sku_id', 'is_default desc,sku_id asc');
            model('community_goods')->update([ 'sku_id' => $first_info[ 'sku_id' ], 'goods_stock' => $goods_stock ], [ [ 'goods_id', '=', $goods_id ] ]);
            ...
            if ($goods_data[ 'timer_on' ] > 0) {
                $cron->addCron(1, 0, "商品定时上架", "CronCommunityGoodsTimerOn", $goods_data[ 'timer_on' ], $goods_id);
            }
            if ($goods_data[ 'timer_off' ] > 0) {
                $cron->addCron(1, 0, "商品定时下架", "CronCommunityGoodsTimerOff", $goods_data[ 'timer_off' ], $goods_id);
            }
            model('community_goods')->commit();
...
//                'sort' => $data['sort'],
            ...
            model('community_goods')->update(array_merge($goods_data, $common_data), [ [ 'goods_id', '=', $goods_id ], [ 'goods_class', '=', $this->goods_class[ 'id' ] ] ]);
            ...
            if (!empty($data[ 'goods_sku_data' ][ 0 ][ 'sku_id' ])) {
                if ($data[ 'spec_type_status' ] == 1) {
                    model('community_goods_sku')->delete([ [ 'goods_id', '=', $goods_id ] ]);
                    ...
                    foreach ($data[ 'goods_sku_data' ] as $item) {
                        ...
                        if (!empty($item[ 'sku_id' ])) {
                            ...
                            model('community_goods_sku')->update(array_merge($sku_data, $common_data), [ [ 'sku_id', '=', $item[ 'sku_id' ] ], [ 'goods_class', '=', $this->goods_class[ 'id' ] ] ]);
                        } else {
                            $sku_id = model('community_goods_sku')->add(array_merge($sku_data, $common_data));
                            $sku_id_arr[] = $sku_id;
                        }
                    }

                    // 移除不存在的商品SKU
                    $sku_id_list = model('community_goods_sku')->getList([ [ 'goods_id', '=', $goods_id ] ], 'sku_id');
                    ...
                    if (!empty($sku_id_list)) {
                        model('community_goods_sku')->delete([ [ 'sku_id', 'in', implode(",", $sku_id_list) ] ]);
                    }
                }
            ...
            //定时上下架
            if ($goods_data[ 'timer_on' ] > 0) {
                $cron->deleteCron([ [ 'event', '=', 'CronCommunityGoodsTimerOn' ], [ 'relate_id', '=', $goods_id ] ]);
                $cron->addCron(1, 0, "商品定时上架", "CronCommunityGoodsTimerOn", $goods_data[ 'timer_on' ], $goods_id);
            }
            if ($goods_data[ 'timer_off' ] > 0) {
                $cron->deleteCron([ [ 'event', '=', 'CronCommunityGoodsTimerOff' ], [ 'relate_id', '=', $goods_id ] ]);
                $cron->addCron(1, 0, "商品定时下架", "CronCommunityGoodsTimerOff", $goods_data[ 'timer_off' ], $goods_id);
            }
/*
            event('CommunityGoodsEdit', [ 'goods_id' => $goods_id, 'site_id' => $data[ 'site_id' ] ]);*/
    ...
    {
        $info = model('community_goods')->getInfo($condition, $field);
    ...
    {
        model('community_goods')->update([ 'sort' => $sort ], [ [ 'goods_id', '=', $goods_id ] ]);
        model('community_goods_sku')->update([ 'sort' => $sort ], [ [ 'goods_id', '=', $goods_id ] ]);
    ...
    {
        model('community_goods')->update([ 'goods_state' => $goods_state ], [ [ 'goods_id', 'in', (string) $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        model('community_goods_sku')->update([ 'goods_state' => $goods_state ], [ [ 'goods_id', 'in', (string) $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        return $this->success(1);
    }
    ...
    public function modifyVerifyState($goods_ids, $verify_state, $verify_state_remark, $website_id = 0)
    {
        ...
        model('community_goods')->update([ 'verify_state' => $verify_state, 'verify_state_remark' => $verify_state_remark ], $condition);
        model('community_goods_sku')->update([ 'verify_state' => $verify_state, 'verify_state_remark' => $verify_state_remark ], $condition);
        return $this->success(1);
    }
    ...
    {
        model('community_goods')->update([ 'is_delete' => $is_delete ], [ [ 'goods_id', 'in', $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        model('community_goods_sku')->update([ 'is_delete' => $is_delete ], [ [ 'goods_id', 'in', $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        //删除商品
        if ($is_delete == 1) {
    ...
    {
        model('community_goods')->update([ 'verify_state_remark' => $verify_state_remark, 'verify_state' => 10, 'goods_state' => 0 ], $condition);
        model('community_goods_sku')->update([ 'verify_state_remark' => $verify_state_remark, 'verify_state' => 10, 'goods_state' => 0 ], $condition);
        return $this->success(1);
    }
    ...
    {
        model("community_goods_sku")->setInc([ [ 'sku_id', '=', $sku_id ] ], 'click_num', 1);
        return $this->success(1);
    }
    ...
    {
        $category_info = model('community_goods_category')->getInfo([ [ 'category_id', '=', $category_id ] ], 'category_id,category_id_1, category_id_2, category_id_3, category_full_name');
        $category_array = [];
        ...
        model('community_goods')->update($data, $condition);
//        model('community_goods_sku')->update($data, $condition);
        return $this->success();
    }
    ...
    public function modifyGoodsShipping($is_free_shipping, $shipping_template, $condition)
    {
        ...
        model('community_goods')->update([ 'is_free_shipping' => $is_free_shipping, 'shipping_template' => $shipping_template ], $condition);
        model('community_goods_sku')->update([ 'is_free_shipping' => $is_free_shipping, 'shipping_template' => $shipping_template ], $condition);
        return $this->success();
    }
    ...
    {
        model('community_goods')->delete([ [ 'goods_id', 'in', $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        model('community_goods_sku')->delete([ [ 'goods_id', 'in', $goods_ids ], [ 'site_id', '=', $site_id ] ]);
        return $this->success(1);
    }
    ...
    {
        $info = model('community_goods')->getInfo([ [ 'goods_id', '=', $goods_id ] ], "*");
        ...
        $info[ 'sku_data' ] = model('community_goods_sku')->getList([ [ 'goods_id', '=', $goods_id ] ], $field);
        return $this->success($info);
    }
    ...
    {
        $info = model('community_goods_sku')->getInfo($condition, $field);
        return $this->success($info);
    }
    ...
    {
        if (empty($field)) {
            $field = 'gs.goods_id,gs.sku_id,gs.goods_name,gs.sku_name,gs.sku_spec_format,gs.price,gs.market_price,gs.discount_price,gs.promotion_type,gs.start_time,gs.end_time,gs.stock,gs.click_num,(gs.sale_num ) as sale_num,gs.collect_num,gs.sku_image,gs.sku_images,gs.goods_id,gs.site_id,gs.goods_content,gs.goods_state,gs.is_free_shipping,gs.goods_spec_format,gs.goods_attr_format,gs.introduction,gs.unit,gs.video_url,g.evaluate,gs.is_virtual,gs.max_buy,gs.min_buy,g.goods_image,g.verify_state';
        }
        $join = [
            [ 'community_goods g', 'g.goods_id = gs.goods_id', 'inner' ],
        ];
        $info = model('community_goods_sku')->getInfo([ [ 'gs.sku_id', '=', $sku_id ], [ 'gs.is_delete', '=', 0 ] ], $field, 'gs', $join);
    ...
    {
        $list = model('community_goods')->getList($condition, $field, $order, '', '', '', $limit);
    ...
    {
        $list = model('community_goods')->pageList($condition, $field, $order, $page, $page_size, $alias, $join, $group);
    ...
    public function editGoodsStockPrice($goods_sku_array)
    {
        ...
        try {
            foreach ($goods_sku_array as $k => $v) {
                $sku_info = model("community_goods_sku")->getInfo([ [ 'sku_id', '=', $v[ 'sku_id' ] ] ], "goods_id,stock");
                ...
                if ($k == 0) {//修改商品中的价格等信息
                    ...
                    model('community_goods')->update($goods_data, [ [ 'goods_id', '=', $sku_info[ 'goods_id' ] ] ]);
                }
                ...
                unset($v[ 'stock' ]);
                model('community_goods_sku')->update($v, [ [ 'sku_id', '=', $v[ 'sku_id' ] ] ]);
     ...
     * @param string $limit
     */
    public function getGoodsSkuList($condition = [], $field = 'sku_id,sku_name,price,stock,sale_num,sku_image,goods_id,goods_name,site_id,site_name,spec_name,max_buy,min_buy', $order = 'create_time desc', $limit = null, $alias = '', $join = '', $group = null)
    {
        $list = model('community_goods_sku')->getList($condition, $field, $order, $alias, $join, '', $limit);
        return $this->success($list);
    }
    ...
    {
        $list = model('community_goods_sku')->pageList($condition, $field, $order, $page, $page_size, $alias, $join, $group);
    ...
    {
        model('commander_goods_attr_index')->delete([ [ 'goods_id', '=', $goods_id ], [ 'app_module', '=', 'shop' ] ]);
        if (!empty($goods_attr_format)) {

            $list = model('community_goods_sku')->getList([ [ 'goods_id', '=', $goods_id ] ], 'sku_id');
            ...
            model('commander_goods_attr_index')->addList($attr_data);
        }
    }
    ...
    public function dealGoodsSkuSpecFormat($goods_id, $goods_spec_format)
    {
        ...
        $sku_list = model('community_goods_sku')->getList([ [ 'goods_id', '=', $goods_id ], [ 'sku_spec_format', '<>', '' ] ], 'sku_id,sku_spec_format', 'sku_id asc');
        if (!empty($sku_list)) {
...
//									匹配数量跟规格总数一致表示匹配成功
                ...
                model('community_goods_sku')->update([ 'goods_spec_format' => json_encode($sku_format) ], [ [ 'sku_id', '=', $v[ 'sku_id' ] ] ]);
            }
        }
    }
    ...
    public function incGoodsSaleNum($sku_id, $num)
    {
        ...
        //增加sku销量
        $res = model("community_goods_sku")->setInc($condition, "sale_num", $num);
        if ($res !== false) {
            $sku_info = model("community_goods_sku")->getInfo($condition, "goods_id");
    ...
    public function decGoodsSaleNum($sku_id, $num)
    {
        ...
        //增加sku销量
        $res = model("community_goods_sku")->setDec($condition, "sale_num", $num);
        if ($res !== false) {
            $sku_info = model("community_goods_sku")->getInfo($condition, "goods_id");
    ...
    {
        $res = model('community_goods')->getCount($condition);
        return $this->success($res);
    }
    ...
    {
        $res = model('community_goods_sku')->getCount($condition);
        return $this->success($res);
    }
    ...
    {
        //查询商品原来的数据用于复制
        $goods_info = model('community_goods')->getInfo($condition, '*');
        if (empty($goods_info))
            return $this->error('', '商品不存在，无法复制！');

        $sku_list = model('community_goods_sku')->getList($condition, '*');
        if (empty($sku_list))
            return $this->error('', '商品不存在，无法复制！');
        ...
        try {
            ...
            $goods_id = model('community_goods')->add($goods_data);
            $sku_arr = [];
            foreach ($sku_list as $k => $v) {
                $sku_data = $v;
...
//                $sku_data[ 'verify_state' ] = 0;
            ...
            // 赋值第一个商品sku_id
            $first_info = model('community_goods_sku')->getFirstData([ 'goods_id' => $goods_id ], 'sku_id', 'is_default desc,sku_id asc');
            model('community_goods')->update([ 'sku_id' => $first_info[ 'sku_id' ] ], [ [ 'goods_id', '=', $goods_id ] ]);
    ...
    {
        model('community_goods')->update([ 'goods_state' => 1 ], $condition);
        model('community_goods_sku')->update([ 'goods_state' => 1 ], $condition);
        return $this->success(1);
    }
    ...
    public function modifyGoodsRecommendWay($recom_way, $goods_ids)
    {
        ...
        model('community_goods')->update([ 'recommend_way' => $recom_way ], [ [ 'goods_id', 'in', $goods_ids ] ]);
        model('community_goods_sku')->update([ 'recommend_way' => $recom_way ], [ [ 'goods_id', 'in', $goods_ids ] ]);
        return $this->success();
    }
    ...
    {
        model('community_goods')->update([ 'goods_state' => 0 ], $condition);
        model('community_goods_sku')->update([ 'goods_state' => 0 ], $condition);
        return $this->success(1);
    }
    ...
    public function getGoodsSpecFormat($sku_ids, $goods_spec_format)
    {
        if (!empty($goods_spec_format) && !empty($sku_ids)) {
            ...
            $sku_spec_format = model('community_goods_sku')->getColumn([ [ 'sku_id', 'in', $sku_ids ] ], 'sku_spec_format');
            $sku_spec_format_arr = [];
    ...
    public function getEmptyGoodsSpecFormat($sku_ids, $sku_id)
    {
        if (!empty($sku_id) && !empty($sku_ids)) {
            ...
            $sku_spec_format = model('community_goods_sku')->getValue([ [ 'sku_id', '=', $sku_id ] ], 'sku_spec_format');
            ...
            $sku_spec_list = model('community_goods_sku')->getList([ [ 'sku_id', 'in', $sku_ids ] ], 'sku_spec_format,sku_id');
    ...
    public function getGoodsStockAlarm($site_id)
    {
        ...
        $count = model('community_goods')->query($sql);
    ...
    {
        $config = new ConfigModel();
        $res = $config->getConfig([['site_id', '=',  0], ['app_module', '=', 'admin'], ['config_key', '=', 'COMMUNITY_WITHDRAW']]);
        if(empty($res['data']['value']))
        {
            //默认数据管理
            $res['data']['value'] = [
                'is_period_settlement' => 1,  //是否账期转账
                'period_type' => 1,           //转账周期类型1.天  2. 周  3. 月
    ...
    public function setShopWithdrawConfig($data)
    {
        ...
        $config = new ConfigModel();
        $res = $config->setConfig($data, '商家转账设置', 1, [['site_id', '=',  0], ['app_module', '=', 'admin'], ['config_key', '=', 'COMMUNITY_WITHDRAW']]);
        ...
        $cron->deleteCron([ [ 'event', '=', 'CommunityWithdrawPeriodCalc' ] ]);
        $cron->addCron('2','1','团长周期结算','CommunityWithdrawPeriodCalc',$execute_time,'0',$data['period_type']);
        return $res;
    }
    ...
    public function getGoodsPrice($sku_id, $member_id)
    {
        ...
        $goods_sku_info = model("community_goods_sku")->getInfo([ [ 'sku_id', '=', $sku_id ] ], 'discount_config,discount_method,price,discount_price');
