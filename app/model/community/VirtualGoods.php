...
{
    $list = model('community_goods_sku')->getList($condition, $field, $order, '', '', '', $limit);
    return $this->success($list);
}
    ...
    private function refreshGoodsAttribute($goods_id, $goods_attr_format)
{
    ...
    if (!empty($goods_attr_format)) {
        ...
        model('community_goods_attr_index')->addList($attr_data);
    }
}
    ...
    public function addGoodsVirtual($site_id, $order_id, $order_no, $sku_id, $sku_name, $code, $member_id, $sku_image)
{
    ...
    $res = model("community_goods_virtual")->add($data);
...
{
    $res = model("community_goods_virtual")->delete($condition);
    if ($res === false)
        return $this->error();
...
{
    $goods_virtual_info = model("community_goods_virtual")->getInfo([ [ "code", '=', $code ], [ "is_veirfy", "=", 0 ] ]);
    ...
    $res = model("community_goods_virtual")->update($data, [ [ "code", '=', $code ], [ "is_veirfy", "=", 0 ] ]);
...
{
    $info = model('community_goods_virtual')->getInfo($condition, $field);
    return $this->success($info);
}
...
{
    $list = model('community_goods_virtual')->pageList($condition, $field, $order, $page, $page_size);
    return $this->success($list);
}
    /************************************************************************* 购买的虚拟产品 end *******************************************************************/
}