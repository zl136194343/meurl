...
class Pay extends BaseModel
{
     ...
     * @return Ambigous <>
     */
    public function pay($pay_type, $out_trade_no, $app_type, $member_id, $return_url = '')
    {

        if (REWRITE_MODULE) {
            $notify_url = addon_url("pay/pay/notify");
        } else {
            $notify_url = request()->domain() . '/pay.php';
        }
        $return_url = empty($return_url) ? addon_url("pay/pay/payreturn", [ 'out_trade_no' => $out_trade_no, 'app_type' => $app_type ]) : $return_url;
     ...
     * 创建支付流水号
     */
    public function createOutTradeNo($unique)
    {
...
//        $no = time() . rand(1000, 9999) . $cache;
        $no = createno('pay_out_trade_no', '', $unique);
        return $no;
    }
