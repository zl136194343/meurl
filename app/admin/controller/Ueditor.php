...
class Ueditor extends Controller {
    ...
    private function upFile($fieldName){
        ...
        if(!empty($_FILES[$fieldName])){//上传成功
            ...
            $info = $upload_service->setPath($upload_path)->file($param);
            if($info['code'] > 0){
                $data=array(
                    'state' => 'SUCCESS',
                    'url' => $info['data']['path'],
    ...
    private function upImage($fieldName){
        ...
        if(!empty($_FILES[$fieldName])){//上传成功
            ...
            $info = $upload_service->setPath("common/images/".date("Ymd"). '/')->image($param);
            if($info['code'] >= 0){
                $data=array(
                    'state' => 'SUCCESS',
    ...
    private function upVideo($fieldName, $size){
        ...
        if(!empty($_FILES[$fieldName])){//上传成功
            ...
            $info = $upload_service->setPath($upload_path)->video($param);
            if($info['code'] > 0){
                $data=array(
                    'state' => 'SUCCESS',
                    'url' => $info['data']['path'],
    ...
    private function saveRemote($config,$fieldName){
        ...
        if(!(file_put_contents($fullName, $img) && file_exists($fullName))){ //移动失败
            ...
            //先拉取到本地在上传到云端
            $upload_service = new UploadModel();
            $file_make = $upload_service->imageMake($file['name']);
            $info = $upload_service->fileCloud($file_make, $dirname);
            if($info['code'] > 0){
