...
class System extends BaseAdmin
{
    ...
    public function refresh()
    {
        try {
            ...
            foreach ($addon_list as $k => $v) {
                $addon_admin_addon_menu_res = $menu->refreshMenu('admin', $v[ 'name' ]);
                $addon_shop_addon_menu_res = $menu->refreshMenu('shop', $v[ 'name' ]);
                if ($v[ 'name' ] == 'city') {
                    // 城市站点管理
                    $addon_city_addon_menu_res = $menu->refreshMenu('city', $v[ 'name' ]);
                }
                var_dump($addon_admin_addon_menu_res);
                var_dump($addon_shop_addon_menu_res);
            }
            var_dump($res);
        } catch (\Exception $e) {
    ...
    {
        $arr = [ '', 'bargain', 'city', 'groupbuy', 'pintuan', 'pointexchange', 'seckill', 'coupon', 'fenxiao', 'live', 'notes', 'store', 'platformcoupon', 'wholesale', 'gift', 'cards', 'egg', 'turntable' ];
        $addon = new Addon();
        foreach ($arr as $k => $v) {
        ...
        // 刷新自定义模板
        $diy_template_model = new DiyTemplate();
        $diy_template_model->refresh();
    ...
    public function executeSql()
    {
        ...
        try {
            if (!empty($sqls)) {
                ...
                $database = new Database();
                $data = array (
                    [ 'table_name' => 'account', 'field' => 'account_no', 'datatype' => 'varchar(255)', 'default' => "''", 'comment' => "'账户记录编号'" ],
                    [ 'table_name' => 'account', 'field' => 'type_name', 'datatype' => 'varchar(255)', 'default' => "''", 'comment' => "'类型'" ],
                    [ 'table_name' => 'album', 'field' => 'app_module', 'datatype' => 'varchar(255)', 'default' => "'shop'", 'comment' => "'应用模块'" ],
                );
                $database->checkTableField($data);
                ...
                //通过旧版的方式 查询sql语句
                $session = session("version_update");
                if (!empty($session)) {
                    //执行数据库
                    $sql = check_bom('upload/upgrade/' . $session[ 'sys_version' ] . '/' . $session[ 'sys_release' ] . '/db_upgrade.sql');
