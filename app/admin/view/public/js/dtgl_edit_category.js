...
/*			categoryFullName.push(data.field.category_name);
			...
			var url = ns.url("admin/dtgl/addCategory");
			if (data.field.category_id) {
				//编辑
				url = ns.url("admin/dtgl/editCategory");
			}
			$.ajax({
				url: url,
				data: data.field,
				...
				success: function (data) {
					...
					if (data.code == 0) {
						location.href = ns.url("admin/dtgl/fllists");
					} else {
						repeat_flag = false;
					}
				}
...
function getCategoryInfo(category_id, callback) {
	$.ajax({
		url: ns.url("admin/goodscategory/getCategoryInfo"),
...
function back() {
	location.href = ns.url("admin/goodscategory/lists")
}