layui.use(['form', 'layer'], function(){
    ...
    (function(){
        ...
        $.ajax({
            type : "post",
            url : ns.url('freeshipping://admin/freeshipping/getarealist'),
            data : {
                level:area_level,
            },
            dataType:'json',
    ...
    function compile_table(){
        ...
        for(var index in submit_data){
            if(count_obj(submit_data[index]) > 0){
                ...
                html += '<span class="right-opt"><span class="opt-update" data-selected="'+ index +'">修改</span>';
                html += '</p>';
                html += '</td>';
                html += '</td>';
                html += '</tr>';
    ...
    form.on('submit(save)', function(form_data){
        ...
        var url =  ns.url('freeshipping://admin/freeshipping/add');
        if(freeshipping_id) url = ns.url('freeshipping://admin/freeshipping/edit');
            ...
            success : function(res) {
                ...
                if (res.code == 0) {
                    layer.confirm('添加成功', {
                        ...
                        btn: ['返回列表', '继续添加'],
                        yes: function(){
                            location.href = ns.url('freeshipping://admin/freeshipping/lists');
                        },
                        btn2: function() {
                            location.href = ns.url('freeshipping://admin/freeshipping/add');
                        }
                    });
                }else{
