...
layui.use(['form', 'layer', 'upload', 'laytpl', 'colorpicker'], function () {
	...
	floorForm.on('submit(save)', function (data) {
		...
		$.ajax({
			url: ns.url("pc://admin/pc/editFloor"),
			data: data.field,
			dataType: 'JSON',
			type: 'POST',
			success: function (res) {
				...
				if (res.code == 0) {
					location.href = ns.url("pc://admin/pc/floor");
				}
				repeatFlag = false;
			}
		});
				...
				var textLayer = floorLayer.open({
					...
					success: function (layero, index) {
						...
						floorForm.on('submit(save_upload)', function (data) {
							//图片删除
							if(!data.field.upload_image) uploadImage.delete();
...
function goodsSelect(callback, selectId, params) {
	...
	var url = ns.url("pc://admin/pc/goodsselect", params);
	//iframe层-父子操作
	floorLayer.open({
			title: "商品选择",
			type: 2,
...
function brandSelect(callback, selectId, params) {
	...
	var url = ns.url("pc://admin/pc/brandselect", params);
	//iframe层-父子操作
	floorLayer.open({
			title: "品牌名称",
			type: 2,
