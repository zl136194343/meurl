...
$(function () {
	...
	layui.use(['form', 'laytpl'], function () {
		...
		form.on('submit(save_attr)', function (data) {
			...
			$.ajax({
				url: ns.url("admin/comsattr/editAttr"),
				data: data.field,
				dataType: 'json',
				type: 'post',
		...
		form.on('submit(save_brand)', function (data) {
			...
			if (deleteBrand.length > 0) {
				//删除已关联的品牌
				$.ajax({
					url: ns.url("admin/comsattr/deleteAttrClassBrand"),
			...
			if (brand_id_arr.length > 0) {
				...
				//添加新关联的品牌
				$.ajax({
					url: ns.url("admin/comsattr/addAttrClassBrand"),
					data: data.field,
					dataType: 'json',
					type: 'post',
					success: function (data) {
		...
		form.on('submit(save_add_attribute)', function (data) {
			...
			// 添加属性
			$.ajax({
				url: ns.url("admin/comsattr/addattribute"),
				data: data.field,
				dataType: 'json',
				type: 'post',
				async: false,
		...
		var table = new Table({
			elem: '#attribute_list',
			url: ns.url("admin/comsattr/getAttributeList"),
			...
			cols: [
				[
					...
					{
						title: `<div class="ns-prompt-block">是否是规格参数
									<div class="ns-prompt">
										...
										<div class="ns-growth-box ns-reason-box ns-reason-growth ns-prompt-box">
											<div class="ns-prompt-con">
												<p>如果是规格参数,在添加或编辑商品时,规格会出现在商品规格项中,否则会作为参数出现在可选择的商品参数模板下的参数列表中。</p>
											</div>
										</div>
									</div>
								</div>`,
		...
		form.on('submit(save_edit_attribute)', function (data) {
			...
			if (deleteAttrValueList.length > 0) {
				...
				// 删除已存在的属性值
				$.ajax({
					url: ns.url("admin/comsattr/deleteAttributeValue"),
			...
			if (isEditAttrValue.length > 0) {
				$.ajax({
					url: ns.url("admin/comsattr/editAttributeValue"),
			...
			// 修改属性
			$.ajax({
				url: ns.url("admin/comsattr/editAttribute"),
				data: data.field,
				dataType: 'json',
				type: 'post',
				async: false,
	...
	$("body").on("click", ".brand-list ul.selected li", function () {
		...
		if (id) {
			//删除已关联品牌需要再次确认
			layerIndex = layer.confirm('当前品牌已关联商品参数，移除后数据会发生不可逆转的行为，请谨慎操作', function () {
				deleteBrand.push(id);
...
function getBrandPageList(page) {
	...
	$.ajax({
		url: ns.url("admin/comsattr/getBrandPageList"),
...
function refreshAttrValueList() {
	...
	for (var i = 0; i < attrValueList.length; i++) {
		...
		h += '<tr>';
		h += '<td><input name="attr_value_name" type="text" value="' + item.attr_value_name + '" data-index="' + i + '" placeholder="请输入参数值名称" lay-verify="attr_value" class="layui-input ns-len-mid" autocomplete="off"></td>';
...
function deleteAttrValue(index) {
	if (attrValueList[index].is_add) {
		...
		//删除已添加的属性值需要再次确认
		layerIndex = layer.confirm('参数值已使用，请谨慎操作', function () {
...
function deleteAttribute(attr_id) {
	...
	layerIndex = layer.confirm('确定要删除吗？', function () {
		...
		$.ajax({
			url: ns.url("admin/comsattr/deleteAttribute"),
...
function modifyAttributeSite(attr_id) {
	layer.confirm('确定要转移到平台吗？', function() {
		$.ajax({
			url: ns.url("admin/comsattr/modifyattributesite"),
			data: {attr_class_id: attr_class_id, attr_id: attr_id},
			dataType: 'JSON',
			type: 'POST',
			success: function (res) {
...
function editAttributePopup(attr_id) {
	...
	$.ajax({
		url: ns.url("admin/comsattr/getAttributeDetail"),
		data: {attr_class_id: attr_class_id, attr_id: attr_id},
		dataType: 'json',
		type: 'post',
		success: function (res) {
...
function addAttributeValue(attr_id, callback) {
	...
	if (addAttrValue.length > 0) {
		...
		$.ajax({
			url: ns.url("admin/comsattr/addAttributeValue"),
