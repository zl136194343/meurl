...
$(function () {
	...
	layui.use(['element', 'laytpl', 'form', 'laydate'], function () {
		...
		form.on("select(goods_attr_class)", function (data) {
			...
			if (data.value) {
				$.ajax({
					url: ns.url("admin/community/getAttributeList"),
		...
		form.on('submit(save)', function (data) {
			...
			if (goodsShopCategoryIds.length > 0 && goodsShopCategoryIds.toString()) {
				...
				data.field.goods_shop_category_ids = '';
			}
			...
			data.field.goods_attr_format = JSON.stringify(goodsAttrFormat);//商品属性格式

			var url = ns.url("admin/community/addGoods");
			if (data.field.goods_id) url = ns.url("admin/community/editGoods");
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data.code == 0) {
						location.href = ns.url("admin/community/lists");
					} else {
						repeat_flag = false;
					}
				}
...
function getSupplierPageList(search_text, index) {
	$.ajax({
		url: ns.url("admin/community/getSupplierPageList"),
...
function getCategoryList(category_id, level, callback) {
	...
	$.ajax({
		url: ns.url("admin/community/getCategoryList"),
...
function getSpecList(search_text, index) {
	...
	$.ajax({
		url: ns.url("admin/community/getSpecList"),
...
function getSpecValueList(attr_id, search_text, index) {
	...
	if (specValueSearchResultArr[index].length == 0) {
		$.ajax({
			url: ns.url("admin/community/getSpecValueList"),
