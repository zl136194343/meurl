...
Order.prototype.cols = [
	{
		...
		template: function (orderitem, order) {
			...
			h += '</div>';
			h += '<div class="info">';
			h += '<a href="' + ns.url("community://shop/order/detail", {order_id: orderitem.order_id}) + '" target="_blank" title="' + orderitem.sku_name + '" class="ns-multi-line-hiding ns-text-color">' + orderitem.sku_name + '</a>';
			return h;
		}
	},
	{
	...
	},
	{
		...
		template: function (orderitem, order) {
			...
			if (orderitem.refund_status_name != '') {
				html += '<br/><a href="' + ns.url("community://shop/orderrefund/detail", {order_goods_id: orderitem.order_goods_id}) + '"  target="_blank" class="ns-text-color">' + orderitem.refund_status_name + '</a>&nbsp;&nbsp;';
			}
			return html;
		}
	},
	...
	},
	{
		...
		template: function (orderitem, order) {
			var h = '';
            h += '<p>';
            h += '<a class="ns-text-color" target="_blank" href="' + ns.url("community://shop/member/editmember?member_id=") + order.member_id + '">' + order.nickname + '</a>';
            h += '</p>';
	...
	},
	{
		...
		merge: true,
		template: function (orderitem, order) {
			var url = "community://shop/order/detail";
			var html = '';
			...
			// html += '<a class="layui-btn" href="' + ns.url("community://shop/orderrefund/detail", {order_goods_id: orderitem.order_goods_id}) + '"  target="_blank">' + orderitem.refund_status_name + '</a>';
			// }
			...
			if(order.order_status == 1){
				html += '<a class="layui-btn" href="javascript:;" onclick="storeDispatch(' + order.order_id + ')">发货</a>';
			}
...
Order.prototype.tbody = function () {
	...
	for (var i = 0; i < this.data.list.length; i++) {
		...
		var orderitemList = order.commander_order_goods;
		console.log(order)
		...
		if (order.delivery_id > 0) tbody += '<span class="order-item-header" style="margin-right:50px;">预计自提时间：' + ns.time_to_date(order.arrive_time) + '</span>';
		if (order.delivery_id > 0) tbody += '<span class="order-item-header" style="margin-right:50px;">配货单号：' + order.delivery_no + '</span>';
		...
		if (order.order_status == -1) {
			...
			tbody += '<a class="layui-btn layui-btn-xs" href="' + ns.url("community://shop/order/detail", {order_id: order.order_id}) + '" target="_blank">查看详情</a>';
