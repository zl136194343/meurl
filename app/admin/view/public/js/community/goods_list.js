...
$(function () {
	...
	$("body").on("click", ".contraction", function () {
		...
		if (open == 1) {
			...
			$(this).children("span").text("-");
			$.ajax({
				url: ns.url("admin/community/getGoodsSkuList"),
	...
	layui.use(['form', 'laytpl', 'element'], function () {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if (data.goods_class == 1) {
						window.open(ns.url("admin/community/editgoods", {"goods_id": data.goods_id}));
					} else {
						window.open(ns.url("admin/virtualgoods/editgoods", {"goods_id": data.goods_id}));
					}
					break;
				case 'delete':
					//删除
				...
				case 'copy':
					...
					break;
				case 'browse_records':
					location.href = ns.url("admin/community/goodsBrowse", {goods_id:data.goods_id});
					break;
		...
		form.on('submit(edit_stock)', function (obj) {
			...
			$.ajax({
				type: "POST",
				url: ns.url("admin/community/editGoodsStockPrice"),
				data: {
					'sku_list': field
				},
				dataType: 'JSON',
...
function refreshTable(status) {
	...
	table = new Table({
		elem: '#goods_list',
		url: ns.url("admin/community/lists"),
		cols: cols,
...
function add() {
	location.href = ns.url('admin/community/addGoods');
}
...
function deleteGoods(goods_ids) {
	layer.confirm('删除后进入回收站，确定删除吗?', function () {
		...
		$.ajax({
			url: ns.url("admin/community/deleteGoods"),
...
function offGoods(goods_ids) {
	...
	$.ajax({
		url: ns.url("admin/community/offGoods"),
...
function onGoods(goods_ids) {
	...
	$.ajax({
		url: ns.url("admin/community/onGoods"),
...
function goodsUrl(data) {
	...
	$.ajax({
		type: "POST",
		url: ns.url("admin/community/goodsUrl"),
		data: {
			'goods_id': data.goods_id
		},
		dataType: 'JSON',
...
function goodsPreview(data) {
	...
	$.ajax({
		type: "POST",
		url: ns.url("admin/community/goodsPreview"),
		data: {
			'goods_id': data.goods_id
		},
		dataType: 'JSON',
...
function getVerifyStateRemark(goods_id, verify_state) {
	$.ajax({
		url: ns.url("admin/community/getVerifyStateRemark"),
		data: {goods_id: goods_id},
		dataType: 'JSON',
		type: 'POST',
		success: function (res) {
...
function refreshVerifyStateCount() {
	$.ajax({
		url: ns.url("admin/community/refreshVerifyStateCount"),
		type: 'POST',
		dataType: 'JSON',
		success: function (res) {
...
function copyGoods(goods_id){
	layer.confirm('确定复制当前商品吗?', function() {
		$.ajax({
			url: ns.url("admin/community/copygoods"),
			data: {goods_id : goods_id},
			dataType: 'JSON',
			type: 'POST',
			success: function(res) {
...
function exportGoods(field){
	layer.confirm('确定按当前条件导出吗?', {title:'商品导出'},function() {
		$.ajax({
			url: ns.url("admin/community/exportgoods"),
			data: field,
			dataType: 'JSON',
			type: 'POST',
			success: function(res) {
				...
				if (res.code >= 0) {
					window.open(ns.url('admin/community/export'));
				}
			}
		});
	}, function () {
...
function batchSetting(){
	...
	$.ajax({
		type: "POST",
		url: ns.url("admin/community/batchGoods"),
		data: field,
		dataType: 'JSON',
		success: function (res) {
			setSub = false;
