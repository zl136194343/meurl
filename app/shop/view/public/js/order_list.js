...
Order.prototype.cols = [
	...
	// },
	{
		...
		template: function (orderitem, order) {
			...
			if (order.order_type == 4 && conceal == 1 && order.order_status == 3) {
			    
			    html += '<a class="layui-btn" href="javascript:;" class="default" onclick="orderVerify('+order.order_id+')">核销</a>  ';
			    
			    /*html += '<a class="layui-btn" href="javascript:;'+ns.url('shop/order/verify',{order_id:order.order_id})+'" class="default" target="_blank">核销</a>';*/
			}
			if (order.order_type == 4  && order.order_status == 22) {
			    //使用中的租赁订单
			    html += '<a class="layui-btn" href="javascript:;" class="default" onclick="order_gh('+order.order_id+')">归还</a>  ';
			    /*html += '<a class="layui-btn" href="javascript:;'+ns.url('shop/order/verify',{order_id:order.order_id})+'" class="default" target="_blank">核销</a>';*/
			}
			if (order.order_type == 4  && order.order_status == 23) {
			    //使用中的租赁订单
			    html += '<a class="layui-btn" href="javascript:;" class="default" onclick="order_tyj('+order.order_id+')">退押金</a>  ';
			    /*html += '<a class="layui-btn" href="javascript:;'+ns.url('shop/order/verify',{order_id:order.order_id})+'" class="default" target="_blank">核销</a>';*/
			}
