...
//                'sort' => $sort,
    ...
    public function getCategoryList()
    {
        ...
        $list = $goods_category_list = $goods_category_model->getShopCategoryList($condition, 'category_id,category_name,pid,level,category_id_1,category_id_2', 'sort asc,category_id desc');
        return $list;
    }
    ...
    public function getCategoryByParent()
    {
        ...
        if (!empty($pid)) {
            ...
            $cate = model('shop')->getInfo([['site_id','=',$this->site_id]],'group_id');
            if ($cate['group_id'] == 2) {
             $condition[] = [ 'category_id', '=', 30 ]; 
            }else{
               
                    $condition[] = [ 'category_id', 'in', ['31','36'] ]; 
               /* }else{
                    $condition[] = [ 'category_id', '=', 36 ]; 
                }*/
        ...
        if (request()->isAjax()) {
            ...
            if (!empty($category_sort_array)) {
                ...
                foreach ($category_sort_array as $k => $v) {
                    $res = $goods_category_model->modifyGoodsShopCategorySort($v[ 'sort' ], $v[ 'category_id' ], $this->site_id);
                }
            } else {
                $res = $goods_category_model->modifyGoodsShopCategorySort($sort, $category_id, $this->site_id);
            }
            return $res;
        }
    ...
    public function getCategoryByDiyView()
    {
        if(request()->isAjax()){
            ...
            //店铺端商品分类
            $shop_goods_category_model = new GoodsShopCategoryModel();
            $condition = [
                ['pid', '=', 0],
                ['site_id','=',$this->site_id]
            ];
            $shop_goods_category_list = $shop_goods_category_model->getShopCategoryList($condition, 'category_id,category_name    ');
            $shop_goods_category_list = $shop_goods_category_list['data'];
            return success(0,'',['goods_category_list' => $goods_category_list,'shop_goods_category_list' => $shop_goods_category_list]);
        }
    }
}