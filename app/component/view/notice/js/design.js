		...
		noticeConHtml += '<div class="layui-form-item ns-icon-radio">';
			...
			noticeConHtml += '<div class="layui-input-block">';
				...
				noticeConHtml += '<ul class="ns-icon">';
					noticeConHtml += '<li v-for="(item, index) in goodsSources" v-bind:k="index" v-if="index < count" :class="[item.value == data.sources ? \'ns-text-color ns-border-color ns-bg-color-diaphaneity\' : \'\']" @click="data.sources=item.value">';
			...
			goodsSources: [
				{
					text: "手动添加公告",
					value: "default",
					src: RESOURCEPATH + "/notice/img/manual.png",
					selectedSrc: RESOURCEPATH + "/notice/img/manual_1"+url+".png"
				},
				{
					text : "选择系统公告",
					value : "diy",
					src: RESOURCEPATH + "/notice/img/goods.png",
					selectedSrc: RESOURCEPATH + "/notice/img/goods_1"+url+".png"
				}
			],
			isShow: false,
		}
	...
	methods: {
		...
		noticeSelect: function(callback, selectId) {
			...
			layui.use(['layer'], function () {
				var url = ns.url("admin/notice/noticeselect", {select_id : selectId.toString()});
				//iframe层-父子操作
				layer.open({
					title: "公告选择",
					type: 2,
