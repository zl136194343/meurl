...
Vue.component("goods-list", {
			...
			goodsSources: [
				{
					...
					selectedIcon: goodsListResourcePath + "/goods_list/img/default_selected_icon"+url+".png"
				},
				{
					text: "商品分类",
					value: "category",
					icon: goodsListResourcePath + "/goods_list/img/category_icon.png",
					selectedIcon: goodsListResourcePath + "/goods_list/img/category_selected_icon"+url+".png"
				},
				{
					text : "手动选择",
					value : "diy",
					icon: goodsListResourcePath + "/goods_list/img/diy_icon.png",
					selectedIcon: goodsListResourcePath + "/goods_list/img/diy_selected_icon"+url+".png"
				}
			],
			categoryList: [],
			isLoad: true,
	...
	methods: {
		...
		selectCategory() {
			var self = this;
			var request_url = post + "/Goodscategory/getCategoryByDiyView";
			if (post == 'city') {
				request_url = 'city/city/Goodscategory/getCategoryByDiyView'
			}
			html = '<div class="goodscategory">';
			$.ajax({
				url: ns.url(request_url),
				dataType: 'JSON', //服务器返回json格式数据
				type: 'POST', //HTTP请求类型
				async: false,
				success: function (res) {
					var data = Object.values(res.data), currIndex = 0;
					html += '<div class="left">';
					if (res.data.goods_category_list.length)
						if (currIndex == 0)
							html += '<div class="catehead ns-text-color">商品分类</div>';
						else
							html += '<div class="catehead >商品分类</div>';
					if (res.data.shop_goods_category_list.length)
						html += '<div class="catehead">店内分类</div>';
					html += '</div>';
					html += '<div class="right">';

					for (var i = 0; i < data.length; i++) {
						html += '<div class="goodscategory_item">';
						for (var index = 0; index < data[i].length; index++) {
							html += '<div class="category-item" data-id="' + data[i][index].category_id + '">' + data[i][index].category_name + '</div>'
						}
						html += '</div>';
					}
				}
			...
			html += '</div>';
			html += '</div>';
			$(".draggable-element[data-index='" + self.data.index + "'] .edit-attribute .goods-category-layer").html(html)
			layer.open({
				type: 1,
				title: '选择分类',
				area: ['630px', '430px'],
				...
				success: function (layero, index) {
					$("body").on("click", ".layui-layer-content .left .catehead", function () {
						var index = $(this).index();
						$(this).addClass("ns-text-color ").siblings().removeClass("ns-text-color");
						$(".layui-layer-content .goodscategory_item:eq(" + index + ")").show().siblings().hide();
					});
					$("body").on("click", ".layui-layer-content .goodscategory_item .category-item", function () {
						$(this).addClass("selected ns-border-color").siblings().removeClass("selected ns-border-color");
					});
					$(".layui-layer-content .goodscategory_item .category-item[data-id='" + self.data.categoryId + "']").click();
				},
				yes: function (index, layero) {
					self.data.categoryName = $(".layui-layer-content .goodscategory_item .category-item.selected").text();
					self.data.categoryId = $(".layui-layer-content .goodscategory_item .category-item.selected").attr('data-id');
					layer.closeAll()
				}
			});
		}
...
goodsTagStyleHtml += '<div v-on:click="data.goodsTag=item.value" v-bind:class="{ \'layui-unselect layui-form-radio\' : true,\'layui-form-radioed\' : (data.goodsTag==item.value) }"><i class="layui-anim layui-icon '+post+'">&#xe63f;</i><div>{{item.text}}</div></div>';
