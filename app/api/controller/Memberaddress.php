...
class Memberaddress extends BaseApi
{
    ...
    public function addThreeParties()
    {
        ...
        $latlng_result = $address_model->getLatlngByAddress(['address' => $this->params['full_address']]);
        if($latlng_result['code'] < 0){
            return $this->response($address_model->error([], '当前地址获取不到坐标!'));
        }
        $data = [
            'member_id' => $token['data']['member_id'],
            'name' => $this->params['name'],
            ...
            'address' => $this->params['address'],
            'full_address' => $this->params['full_address'],
            'longitude' => $latlng_result['data']['longitude'] ?? '',
            'latitude' => $latlng_result['data']['latitude'] ?? '',
