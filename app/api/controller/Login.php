...
class Login extends BaseApi
{
    ...
    public function login()
    {
        ...
        // 校验验证码
        if($config_info['shop_reception_login'] == 1){
            $captcha = new Captcha();
...
//        $hospital_id = model('member')->getInfo([['mobile','=',$this->params[ "mobile" ]]],'hospital_id');
//                if (empty($hospital_id['hospital_id'])) {
//                    //说明没有数据
//                    $da = $this->getadd($this->params[ "mobile" ]);
//                    /*file_put_contents('test.txt',$da);*/
//                     $da = json_decode($da,true);
//                   $listes = [];
//                    if (!empty($da)) {
//                        $listes['hospital_id']= $da['data']['terminalNo']??0;
//                        $listes['hospital_name']= $da['data']['schoolName'];
//                    }
//                    //将数据存入数据库
//                    if(!empty($listes)){
//                        model('member')->update($listes,[['mobile','=',$this->params[ "mobile" ]]]);
//                    }
//
//                }
