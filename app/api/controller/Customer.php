    ...
    public function getoperationallist()
    {
         ...
         if ($token[ 'code' ] < 0) return $this->response($token);
            ...
            $condition = [
                ...
                [ 'a.counterpart_id', '=', $this->member_id??2365 ]
            ];
            if (!empty($mobile)) {
                $condition[] = ['a.moblile','like','%'.$mobile.'%'];
            }
            $join = [['member m','a.member_id = m.member_id','left']];
             $list = model('customer_operational')->PageList($condition,'a.*,m.headimg','',$page_index,$page_size,$alias = 'a', $join);
             return $this->response($this->success($list));
    }
