...
class Order extends BaseApi
{
    ...
    public function addd12()
    {
       
        
        $message_model = new WeappModel();
        $order_common_model = new ShopSettlement();
        $data = input();
        $res = $order_common_model->shopSettlement(1667391200);
        return $res;
    }
    
    public function addd13()
    {
        
        $message_model = new Customer();
        $res =  $message_model->shopCustomer(1668700800);
        
        return $res;
    }
    public function addd123()
    {
        
        $message_model = new OrderCommonModel();
        $res =  $message_model->orderOnlinePay(['out_trade_no'=>'20221102111323680001']);
        //dump($res);die;
        return $res;
    }
    ...
    public function lists()
    {
        ...
        switch ( $order_status ) {
            ...
            case "waitrate"://待评价
                ...
                break;
            case "zulinzhong"://租赁中
                $condition[] = [ "order_status", "=", 22 ];
                
                break;
           case "yiguihuan"://已归还
                $condition[] = [ "order_status", "=", 23 ];
                
                break;
           case "zlqbdd"://租赁全部订单
                $condition[] = [ "order_status",  "in", [ 4, 10 ,22,23,0,3]];
...
//		}
        ...
        $is_lease = isset($this->params[ 'is_lease' ]) ? $this->params[ 'is_lease' ] : 0;
        if ($is_lease != 0) {
            $condition[] = [ "is_lease", "=", 1 ];
        }else{
             $condition[] = [ "is_lease", "=", 0 ];
        }
    ...
    public function takeDelivery()
    {
        ...
        $result = $order_model->orderCommonTakeDelivery($take_condition,$this->member_id);
        return $this->response($result);
    }
