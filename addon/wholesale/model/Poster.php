...
class Poster extends BaseModel
{
    ...
    public function goods($app_type, $page, $qrcode_param, $promotion_type = 'null')
    {
        try {
            ...
            $res = $option_res->jpeg('upload/poster/goods', 'goods_' . $promotion_type . '_' . $qrcode_param['goods_id'] . '_' . $qrcode_param['source_member'] . '_' . $app_type);
            return $res;
        } catch ( \Exception $e ) {
    ...
    {
        $join = [
            ['goods_sku sku', 'wgs.sku_id = sku.sku_id', 'inner'],
        ];
        $field = 'wgs.min_price,wgs.max_price,wgs.min_num,sku.sku_name,sku.sku_image';
        $info = model('wholesale_goods_sku')->getInfo(['wgs.goods_id' => $goods_id], $field, 'wgs', $join);
        return $info;
    }
    ...
    private function getGoodsQrcode($app_type, $page, $qrcode_param, $promotion_type = 'null')
    {
        $res = event('Qrcode', [
            ...
            'qrcode_name' => 'goods_' . $promotion_type . '_' . $qrcode_param['goods_id'] . '_' . $qrcode_param['source_member'],
        ], true);
        return $res;
    }
}