...
class Order extends BaseModel
{
    ...
    public function orderPay($data)
    {
        ...
        try {
            ...
            switch ( $order_info[ 'type' ] ) {
                case 1://礼品
                    ...
                    $gift_data = array (
                        ...
                        'get_type_name' => '积分兑换',
                        'tag' => 'pointexchange',
                        'relevant_order_id' => $order_info[ "order_id" ]
                    );
                    $res = $gift_model->addOrder($gift_data, 1);
                    $gift_order_id = $res['data'];
                    model("promotion_exchange_order")->update(['gift_order_id' => $gift_order_id], [ [ "order_id", "=", $order_info[ "order_id" ] ] ]);
                    break;
                case 2://优惠券
                    $coupon_model = new Platformcoupon();
                    //循环多次领用平台优惠券
                    $start = 1;
                    $coupon_num = $order_info[ "num" ];
                    if ($coupon_num > 0) {
                        while ($start <= $coupon_num) {
            ...
            model('promotion_exchange')->setInc([['id','=',$order_info['exchange_id']]],'sales_volume',1);
