...
Evaluate.prototype.getList = function(d) {
	...
	$.ajax({
		url: ns.url("city/city/goods/evaluateList"),
		data: {
			"page": page,
			"limit": limit,
		...
		success: function (res) {
			...
			for (var i in d) {
				...
				if (d[i].explain_type == 1) {
					html += `<p class="evaluate-level-good"><img src= "${CITYIMG}/good_evaluate.png" /><span>好评</span></p>`;
				} else if (d[i].explain_type == 2) {
					html += `<p class="evaluate-level-middel"><img src= "${CITYIMG}/middel_evaluate.png" /><span>中评</span></p>`;
				} else {
					html += `<p class="evaluate-level-bad"><img src= "${CITYIMG}/bad_evaluate.png" /><span>差评</span></p>`;
				}
...
layui.use(['form', 'laypage', 'layer'], function() {
	...
	$(".ns-del-eval .layui-btn").click(function () {
		...
		$.ajax({
			url: ns.url("city/city/goods/deleteEvaluate"),
			data: {
				"id": id
			},
			type: "POST",
