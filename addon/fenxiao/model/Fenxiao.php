...
class Fenxiao extends BaseModel
{
    ...
    public function addFenxiao($data)
    {
        ...
        try {
            ...
            //修改会员信息  申请成为分销商但自己不能是自己的上级
            
            if (!empty($data[ 'parent' ])){
                //当存在推荐人时 推荐人为对应人的上级
                model('member')->update([ 'fenxiao_id' => $data[ 'parent' ], 'is_fenxiao' => 1 ], [ [ 'member_id', '=', $data[ 'member_id' ] ] ]);
                //并将对应的上级人数减一
                 
            }else{
                //当不存在推荐人时 没有上级
                model('member')->update([ 'fenxiao_id' => 0, 'is_fenxiao' => 1 ], [ [ 'member_id', '=', $data[ 'member_id' ] ] ]);
            }
    ...
    {
        $condition[] = [ 'f.is_delete', '=', 0 ];
        $field = 'f.*,pf.fenxiao_name as parent_name,m.username';
        $alias = 'f';
        $join = [
            [
                'fenxiao pf',
    ...
    public function getFenxiaoTeam($level, $fenxiao_id, $page = 1, $page_size = PAGE_LIST_ROWS)
    {
        ...
        switch ( $level ) {
            case 0:
                ...
                break;
            case 1:
                $condition = "f.parent = {$fenxiao_id} AND f.is_delete = 0";
                break;
            case 2:
    ...
    public function getFenxiaoTeamNum($fenxiao_id)
    {
        ...
        switch ( $level ) {
            case 2:
                $data = model('fenxiao')->query("SELECT GROUP_CONCAT(fenxiao_id) as fenxiao_id FROM {$prefix}fenxiao WHERE parent = {$fenxiao_id} and is_delete = 0");
                if (isset($data[ 0 ]) && isset($data[ 0 ][ 'fenxiao_id' ]) && !empty($data[ 0 ][ 'fenxiao_id' ])) {
                    $fenxiao_id = $data[ 0 ][ 'fenxiao_id' ];
                ...
                break;
            case 3:
                $two_data = model('fenxiao')->query("SELECT GROUP_CONCAT(fenxiao_id) as fenxiao_id FROM {$prefix}fenxiao WHERE parent = {$fenxiao_id} and is_delete = 0");
                if (isset($two_data[ 0 ]) && isset($two_data[ 0 ][ 'fenxiao_id' ]) && !empty($two_data[ 0 ][ 'fenxiao_id' ])) {
                    ...
                    $three_data = model('fenxiao')->query("SELECT GROUP_CONCAT(fenxiao_id) as fenxiao_id FROM {$prefix}fenxiao WHERE parent in ({$two_fenxiao_ids}) and is_delete = 0");
...
//     		}
    ...
    private function directlyBecomeFenxiao($member_id)
    {
        ...
        if (!empty($member_info)) {
            ...
            $level_info = $level_model->getLevelInfo([ [ 'is_default', '=', 1 ] ], 'level_id,level_name');
    ...
    public function bindRelation($member_id)
    {
        ...
        /*file_put_contents('text.txt',json_encode($member_info));*/
     ...
     * @param unknown $member_id
     */
    public function bindRelationT($member_id,$source_member)
    {
        $member_info = model('member')->getInfo([ [ 'member_id', '=', $member_id ], [ 'fenxiao_id', '=', '' ] ], 'member_id,source_member');
        
        /*file_put_contents('text.txt',json_encode($member_info));*/
        if (!empty($member_id)&&!empty($member_info)) {
             
            // 查询推荐人是否是分销商
            if (!empty($source_member)) {
                //说明有推荐人
               $fenxiao_info = model('fenxiao')->getInfo([ [ 'member_id', '=', $source_member ], [ 'is_delete', '=', 0 ] ], 'fenxiao_id,parent');
            }else{
                //没有推荐人
                $fenxiao_info['fenxiao_id'] = 0;
            }
    ...
    public function fenxiaoUpgrade($fenxiao_id)
    {
        ...
        $field = 'f.level_id,m.order_num,m.order_money,f.one_fenxiao_order_num,f.one_fenxiao_order_money,f.one_child_num,f.one_child_fenxiao_num,fl.one_rate,fl.level_num';
        $fenxiao_info = model('fenxiao')->getInfo(
            [ [ 'f.fenxiao_id', '=', $fenxiao_id ], [ 'f.status', '=', 1 ], [ 'f.is_delete', '=', 0 ] ], $field, 'f', $join
        );
    ...
    public function geFenxiaoLastLevel($member_id)
    {
        ...
        $fenxiao_info = model('fenxiao')->getInfo([ [ 'f.member_id', '=', $member_id ], [ 'f.status', '=', 1 ], [ 'f.is_delete', '=', 0 ] ], 'f.level_id,m.order_num,m.order_money,f.one_fenxiao_order_num,f.one_fenxiao_order_money,f.one_child_num,f.one_child_fenxiao_num,fl.one_rate,fl.level_num', 'f', $join);
     ...
     * @return array
     */
    public function CronMemberCancel($member_id)
    {
        $info = model('fenxiao')->getInfo([ [ 'member_id', '=', $member_id ] ]);
        if (empty($info)) {
            return $this->success();
        }
