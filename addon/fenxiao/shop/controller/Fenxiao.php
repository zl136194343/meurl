    ...
    public function lists()
    {
        ...
        if (request()->isAjax()) {
            ...
            foreach ($res['data']['list'] as &$r){
                $fgs = model('fenxiao_goods_sku')->getInfo([['goods_id','=',$r['goods_id']]],'one_rate,one_money');
                if (empty($fgs['one_rate'])) {
                    $r['one_rate'] =0;
                }else{
                    $r['one_rate'] = $fgs['one_rate'];
                }
    ...
    public function config()
    {
        ...
        if (request()->isAjax()) {
            ...
            try {
                ...
                if ($is_fenxiao) {
                    foreach ($fenxiao_price as $sku_id => $fenxiao_price) {
                        if (empty($fenxiao_price) || $fenxiao_price < 0) $fenxiao_price = 0;
                        $res = model('goods_sku')->update([ 'fenxiao_price' => $fenxiao_price ], [ [ 'sku_id', '=', $sku_id ], [ 'site_id', '=', $this->site_id ] ]);
                    }
                }
        ...
        $fenxiao_config = $fenxiao_config_model->getFenxiaoBasicsConfig()['data'] ?? [];
