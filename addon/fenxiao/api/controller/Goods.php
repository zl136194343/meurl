    ...
    public function detail()
    {
        ...
        if ($res[ 'data' ]) {
            ...
            if($fenxiao_info['parent'] != 0){
                 $two_rate = model('fenxiao')->getInfo([['fenxiao_id','=',$fenxiao_info['parent']]],'two_rate');
                 if ($v[ 'one_rate' ] > 0) {
                        $money = number_format($discount_price * $v[ 'one_rate' ] / 100*$two_rate['two_rate']/ 100, 2);
                    } elseif ($v[ 'one_money' ] > 0) {
                        $money = $v[ 'one_money' ]*$two_rate['two_rate']/ 100;
                    }
             }else{
                 if ($v[ 'one_rate' ] > 0) {
...
//		拿到商品属性，查询sku_id
        ...
        $fenxiao_model = new FenxiaoModel();
        $fenxiao_info = $fenxiao_model->getFenxiaoInfo([ [ 'member_id', '=', $this->member_id??30 ] ], "fenxiao_id,level_id,parent");
        $fenxiao_info = $fenxiao_info[ 'data' ];
...
//		$basics_config = $basics_config[ 'data' ][ 'value' ];
        ...
        if($fenxiao_info['parent'] != 0){
            $join[] = 
                    [ 'fenxiao_goods_collect fg', 'fg.goods_id = g.goods_id', 'inner' ];
        }
        ...
        $field = 'fgs.goods_sku_id,fgs.goods_id,fgs.sku_id,fgs.level_id,fgs.one_rate,fgs.one_money,fgs.two_rate,fgs.two_money,fgs.three_rate,fgs.three_money,gs.sku_name,gs.discount_price,gs.price,gs.stock,gs.sale_num,gs.sku_image,gs.site_id,gs.recommend_way,g.goods_image,g.goods_name';
