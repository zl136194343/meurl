...
class GroupbuyOrderCreate extends OrderCreate
{
    ...
    public function calculate($data)
    {
        ...
        $groupbuy_info_result = $groupbuy_model->getGroupbuyInfo([ [ "groupbuy_id", "=", $groupbuy_id ] ]);
        ...
        $shop_goods_list['promotion_id'] = $groupbuy_info['groupbuy_id'];
        //判断活动是否过期或开启
        ...
        //判断购买数是否超过限购
        if ($groupbuy_info[ "buy_num" ] > $data[ 'num' ]) {
            $this->error = 1;
...
//        Cache::tag("order_create_groupbuy_".$data['member_id'])->set("order_create_groupbuy_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
    ...
    public function getGroupbuyGoodsInfo($data)
    {
        ...
        $field = 'sku_id,sku_name, sku_no,
            ...
            is_free_shipping, shipping_template, goods_class, goods_class_name, goods_id,sku_spec_format,goods_name';

        $info = model("goods_sku")->getInfo([ [ 'sku_id', '=', $data[ "sku_id" ]],['is_delete','=',0] ], $field);
        $shop_goods_list = [];
