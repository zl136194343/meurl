...
class Poster extends BaseModel
{
    ...
    {
        try {
            $goods_info = $this->getGoodsInfo($qrcode_param[ 'groupbuy_id' ]);
            ...
            $option = [
                ...
                [
                    ...
                    'data' => [
                        !empty($goods_info[ 'goods_image' ]) ? $goods_info[ 'goods_image' ] : $default_goods_img,
                        100,
                        134,
                        539,
                        539,
            ...
            $res = $option_res->jpeg('upload/poster/goods', 'goods_' . $promotion_type . '_' . $qrcode_param[ 'groupbuy_id' ] . '_' . $qrcode_param[ 'source_member' ] . '_' . $app_type);
            return $res;
        } catch (\Exception $e) {
    ...
    private function getGoodsInfo($id)
    {
        ...
        $condition = [
            [ 'pg.groupbuy_id', '=', $id ], [ 'g.goods_state', '=', 1 ], [ 'g.is_delete', '=', 0 ]
        ];
        $info = model('promotion_groupbuy')->getInfo($condition, $field, $alias, $join);
        if (!empty($info) && !empty($info[ 'goods_image' ])) {
            $goods_image = explode(',', $info[ 'goods_image' ]);

            $info[ 'goods_image' ] = $goods_image[ 0 ];
        }
        return $info;
    }
    ...
    private function getGoodsQrcode($app_type, $page, $qrcode_param, $promotion_type = 'null')
    {
        $res = event('Qrcode', [
            ...
            'qrcode_name' => 'goods_' . $promotion_type . '_' . $qrcode_param[ 'groupbuy_id' ] . '_' . $qrcode_param[ 'source_member' ],
        ], true);
        return $res;
    }
}