 ...
 * 供货商控制器基类
 */
class BaseSupply extends Controller
{
    protected $init_menu = [];
    protected $crumbs = [];
    ...
    public function __construct()
    {
        ...
        $supply_info       = $supply_model->getSupplierInfo([['supplier_site_id', '=', $this->supply_id]]);
        $this->supply_info = $supply_info['data'];
        $this->website_id  = $supply_info['data']['website_id'] ?? 0;


        if (empty($this->uid)) {
            $this->redirect(addon_url("supply://supply/login/login"));
            exit();
        }
        if ($this->supply_id == 0) {
            $this->redirect(addon_url("supply://supply/apply/index"));
            exit();
        }
        //检测用户组
        $this->getGroupInfo();
        ...
        $this->assign('site_id',$this->supply_id);
    ...
    private function initBaseInfo()
    {
        ...
        //获取信息
        $this->assign("supply_info", $this->supply_info);
        //加载菜单树
    ...
    private function getGroupInfo()
    {
        ...
        $group_info_result = $group_model->getGroupInfo([["group_id", "=", $this->user_info["group_id"]], ["site_id", "=", $this->supply_id], ["app_module", "=", $this->app_module]]);
    ...
    {
        $user = new UserModel();
        $user->addUserLog($this->uid, $this->user_info['username'], $this->supply_id, $action_name, $data);
    }
}
