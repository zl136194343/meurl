 ...
 */
class Express extends BaseSupply
{
    ...
    public function expressCompany()
    {
        ...
        //供应商物流公司
        $company_shop_list_result = $express_company_model->getExpressCompanyShopList([ [ "site_id", "=", $this->supply_id ] ]);
        $temp_company_shop_list = $company_shop_list_result[ "data" ];
        $company_shop_list = [];
        //删除已有的公用物流公司
        if (!empty($temp_company_shop_list)) {
            foreach ($temp_company_shop_list as $k => $v) {
                //供应商物流公司存在的话,删除掉公共物流公司中对应的数据
        ...
        $this->assign("company_shop_list", $company_shop_list);//供应商物流公司
        //供应商配置的物流公司
    ...
    public function editTemplate()
    {
        ...
        if (request()->isAjax()) {
            ...
            $this->assign('fee_type_json', json_encode($fee_type_obj));
            $this->assign('area_level', $area_level);//地址级别
            $info_result = $express_template_model->getExpressTemplateInfo($template_id, $this->supply_id);
    ...
    public function deleteTemplate()
    {
        if (request()->isAjax()) {
            $template_id = input("template_id", 0);
            $express_template_model = new ExpressTemplate();
            $result = $express_template_model->deleteExpressTemplate($template_id, $this->supply_id);
            return $result;
        }
    }
    ...
    public function defaultTemplate()
    {
        if (request()->isAjax()) {
            $template_id = input("template_id", 0);
            $express_template_model = new ExpressTemplate();
            $result = $express_template_model->updateDefaultExpressTemplate($template_id, 1, $this->supply_id);
            return $result;
        }
    }
    ...
    public function getShopExpressCompanyList()
    {
        if (request()->isAjax()) {
            $express_company_model = new ExpressCompany();
            //供应商物流公司
            $result = $express_company_model->getExpressCompanyShopList([ [ "site_id", "=", $this->supply_id ] ]);
            return $result;
        }
    }
}
