 ...
 * 商品类型/属性管理 控制器
 */
class Goodsattr extends BaseSupply
{
    /**
     * 商品类型列表
     */
    ...
    public function editAttribute()
    {
        if (request()->isAjax()) {
            ...
            $attr_value_list = input('attr_value_list', "");// 属性值列表（'',''隔开注意键值对）
            $attr_type = input('attr_type', 0);// 属性类型  （1.单选 2.多选3. 输入 注意输入不参与筛选）
            $site_id = input('site_id', $this->supply_id);// 站点id
    ...
    public function getAttributeList()
    {
        if (request()->isAjax()) {
            $attr_class_id = input('attr_class_id', 0);// 商品类型id
            $goods_attr_model = new GoodsAttributeModel();
            return $goods_attr_model->getAttributeList([ [ 'site_id', 'in', ( "0,$this->supply_id" ) ], [ 'attr_class_id', '=', $attr_class_id ] ]);
        }
    }
