...
Order.prototype.cols = [
    {
        ...
        template : function(item){
            ...
            h += '<a  href="javascript:void(0);" onclick=goodsPreview('+ item.goods_id +') title="' + item.sku_name + '">' + item.sku_name + '</a>';
            h += '</div>';
            return h;
        }
    },
    ...
    },
    {
        ...
        merge : true,
        template : function(item){
            var html = '<div class="ns-table-btn"><a class="layui-btn" href="'+ ns.url("supply://supply/orderrefund/detail",{order_goods_id:item.order_goods_id})+'"  target="_blank">查看详情</a></div>';
            return html;
...
function goodsPreview(data) {
	...
	$.ajax({
		type: "POST",
		url: ns.url("supply://supply/goods/goodsPreview"),
		data: {
			'goods_id': data
		},
		dataType: 'JSON',
