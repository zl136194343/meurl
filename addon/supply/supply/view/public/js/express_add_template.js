layui.use(['form', 'layer'], function(){
    ...
    (function(){
        ...
        $.ajax({
            type : "post",
            url : ns.url('supply://supply/express/getarealist'),
            data : {
                level:area_level,
            },
            dataType:'json',
    ...
    function compile_table(){
        ...
        for(var index in submit_data){
            if(count_obj(submit_data[index]) > 0){
                ...
                html += '<span class="right-opt"><span class="opt-update" data-selected="'+ index +'">修改</span>&nbsp;<span class="opt-delete" data-selected="'+ index +'">删除</span></span>';
                html += '</p>';
                html += '</td>';
    ...
    function get_modal_html(){
        var modal_html = `<div class="area-modal">
    ...
    form.on('submit(save)', function(form_data){
        ...
        var url =  ns.url('supply://supply/express/addtemplate');
        if(template_id) url = ns.url('supply://supply/express/editTemplate');
        ...
        $.ajax({
            ...
            success : function(res) {
                layer.msg(res.message, {}, function(){if(res.code == 0){window.location.href = ns.url('supply://supply/express/template');}});
            }
        });
