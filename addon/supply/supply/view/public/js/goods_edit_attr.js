...
$(function () {
	...
	layui.use(['form', 'laytpl'], function () {
		...
		form.on('submit(save_add_attribute)', function (data) {
			...
			// 添加属性
			$.ajax({
				url: ns.url("supply://supply/goodsattr/addAttribute"),
				data: data.field,
				dataType: 'json',
				type: 'post',
				async: false,
		...
		var table = new Table({
			elem: '#attribute_list',
			url: ns.url("supply://supply/goodsattr/getAttributeList"),
		...
		form.on('submit(save_edit_attribute)', function (data) {
			...
			if (deleteAttrValueList.length > 0) {
				...
				// 删除已存在的属性值
				$.ajax({
					url: ns.url("supply://supply/goodsattr/deleteAttributeValue"),
			...
			if (isEditAttrValue.length > 0) {
				$.ajax({
					url: ns.url("supply://supply/goodsattr/editAttributeValue"),
			...
			// 修改属性
			$.ajax({
				url: ns.url("supply://supply/goodsattr/editAttribute"),
				data: data.field,
				dataType: 'json',
				type: 'post',
				async: false,
...
function refreshAttrValueList() {
	...
	for (var i = 0; i < attrValueList.length; i++) {
		...
		h += '<tr>';
		h += '<td><input name="attr_value_name" type="text" value="' + item.attr_value_name + '" data-index="' + i + '" placeholder="请输入参数名称" lay-verify="attr_value" class="layui-input ns-len-mid"></td>';
		h += '<td><input name="attr_value_sort" type="number" value="' + item.sort + '" data-index="' + i + '" lay-verify="num" class="layui-input ns-len-small"></td>';
...
function deleteAttribute(attr_id) {
	...
	layerIndex = layer.confirm('确定要删除吗？', function () {
		...
		$.ajax({
			url: ns.url("supply://supply/goodsattr/deleteAttribute"),
...
function editAttributePopup(attr_id) {
	$.ajax({
		url: ns.url("supply://supply/goodsattr/getAttributeDetail"),
		data: {attr_class_id: attr_class_id, attr_id: attr_id},
		dataType: 'json',
		type: 'post',
		success: function (res) {
...
function addAttributeValue(attr_id, callback) {
	...
	if (addAttrValue.length > 0) {
		...
		$.ajax({
			url: ns.url("supply://supply/goodsattr/addAttributeValue"),
