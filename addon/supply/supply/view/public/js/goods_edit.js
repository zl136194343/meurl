    ...
    layui.use(['element', 'laytpl', 'form', 'laydate'], function () {
        ...
        form.on("switch(spec_type)", function (data) {
            ...
            if (status) {
                ...
                $("input[name='goods_stock_alarm']").removeAttr("disabled", true).val("");
            }
        });
        ...
        form.on("select(goods_attr_class)", function (data) {
            ...
            if (data.value) {
                $.ajax({
                    url: ns.url("supply://supply/goods/getAttributeList"),
        ...
        var upload = new Upload({
            elem: '#videoUpload',
            url: ns.url("supply://supply/upload/video"),
            accept: "video",
        ...
        form.verify({
            ...
            //销售阶梯价
            ladder_price: function (value, item) {
                if (!$("input[name='spec_type']").is(":checked")) {
                    if (value <= 0) {
                        return "销售阶梯价的价格不能小于或等于0";
                    }
                }
            },
            //销售阶梯数量
            ladder_number: function (value, item) {
                if (!$("input[name='spec_type']").is(":checked")) {
                    if (value <= 0) {
                        return "销售阶梯价的数量不能小于或等于0";
                    }
                }
            },
            //sku销售阶梯价
            sku_ladder_price: function (value, item) {
                if ($("input[name='spec_type']").is(":checked")) {
                    if (value <= 0) {
                        return "销售阶梯价的价格不能小于或等于0";
                    }
            ...
            //sku销售阶梯数量
            sku_ladder_number: function (value, item) {
                if ($("input[name='spec_type']").is(":checked")) {
                    if (value <= 0) {
                        return "销售阶梯价的数量不能小于或等于0";
                    }
        ...
        form.on('submit(save)', function (data) {
            ...
            if (spec_type == 0) {
                ...
                //整理商品阶梯价格
                var item_list = $(".common-ladder-block");
                var price_array = {};

                var item_price_list = item_list.find('tr');

                if (item_price_list.length == 0) {
                    layer.msg('规格阶梯价格不能为空！', {icon: 5, anim: 6});
                    return;
                }
                var first_item = item_list.find('tr:eq(0)');
                var second_item = item_list.find('tr:eq(1)');
                ...
                var price_json = JSON.stringify(temp_list);
                ...
                var item_list = $(".sku-ladder-block");
                var price_array = {};
                item_list.each(function () {
                    ...
                    goodsSkuData[index].price_json = JSON.stringify(temp_list);
                });
            ...
            var url = ns.url("supply://supply/goods/addGoods");
            if (data.field.goods_id) url = ns.url("supply://supply/goods/editGoods");
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.code == 0) {
                        location.href = ns.url("supply://supply/goods/lists");
                    } else {
                        repeat_flag = false;
                    }
                }
...
function refreshSkuTable() {
    ...
    for (var j = 0; j < goodsSkuData.length; j++) {
        ...
        var item_goods_sku_data = goodsSkuData[j];

        var price_array = item_goods_sku_data.price_json != '' ? JSON.parse(item_goods_sku_data.price_json) : [{
            num: '',
            price: ''
        }];
        var column_num_array = arrayColumn(price_array, 'num');
        var column_price_array = arrayColumn(price_array, 'price');
        //阶梯价格(arrayColumn)
        goodsSkuData[j]['column_num_array'] = column_num_array;
        goodsSkuData[j]['column_price_array'] = column_price_array;
    }
    ...
    laytpl(sku_template).render(data, function (html) {
        ...
        $(".sku-table .layui-input-block input").keyup(function () {
            ...
            if ($(this).parents('.sku-ladder-block').length == 0) {
                var index = $(this).attr("data-index");
                ...
                //整理商品阶梯价格
                var item_list = $(this).parents('.sku-ladder-block');
                var item_index = item_list.attr("data-index");
                var price_array = {};
                ...
                goodsSkuData[item_index]['price_json'] = price_json;
            }
        }).blur(function () {
            $(this).keyup();
        });
    ...
    if ($("input[name='spec_type']").is(":checked")) {
        ...
        $(".js-edit-sku-list>div").each(function () {
            var item = {
                ...
                price_json: $(this).children("input[name='edit_price_json']").val(),
...
function getCategoryList(category_id, level, callback) {
    ...
    $.ajax({
        url: ns.url("supply://supply/goods/getCategoryList"),
...
function getSpecList(search_text, index) {
    ...
    $.ajax({
        url: ns.url("supply://supply/goods/getSpecList"),
...
function getSpecValueList(attr_id, search_text, index) {
    ...
    if (specValueSearchResultArr[index].length == 0) {
        $.ajax({
            url: ns.url("supply://supply/goods/getSpecValueList"),
...
function addPricemodel(obj) {
    ...
    if (!$("input[name='spec_type']").is(":checked")) {
        var price_verify = 'ladder_number';
        var number_verify = 'ladder_price';
    } else {
        var price_verify = 'sku_ladder_number';
        var number_verify = 'sku_ladder_price';
    }
    $(obj).parent().before('<td><input type="number" class="layui-input goods-input-len num" lay-verify="' + price_verify + '" autocomplete="off" name="num[]" /></td>');
    var index = $(obj).parent().index();
    $(obj).parent().parent().next().append('<td><input type="number" class="layui-input goods-input-len price" lay-verify="' + number_verify + '" autocomplete="off" name="price[]"/></td>');
