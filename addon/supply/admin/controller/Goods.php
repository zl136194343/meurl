...
use addon\supply\model\goods\GoodsCategory as GoodsCategoryModel;
/**
 * 商品管理 控制器
 */
class Goods extends BaseSupply
{
    ...
    public function editCategory()
    {
        ...
        if (request()->isAjax()) {
            ...
            $supply_commission_rate = input('supply_commission_rate', ''); // 佣金比率%

            $condition = [
               [ 'category_id','=',$category_id]
            ];
            $supply_goods_category = new GoodsCategoryModel;
            $res = $supply_goods_category->editCategory($condition,$supply_commission_rate);
            return $res;
        } else {
            $category_id = input('category_id', ''); // 分类id
            if (empty($category_id)) {
            ...
            return $this->fetch('goods/edit_category');
        }
    }
}
