...
Order.prototype.cols = [
    {
        title : '<input type="checkbox" name="sub" lay-skin="primary"><span class="ns-line-hiding" title="商品信息" style="margin-left:10px;">商品信息</span>',
        width : "20%",
    ...
    {
        title : '<span class="ns-line-hiding" title="供应商名称">供应商名称</span>',
        width : "10%",
        align : "center",
        className : "shop-info",
        merge : true,
        template : function(item){
            var h = '<div>';
            h += '<span class="ns-line-hiding" title="'+ item.site_name +'">' + item.site_name + '</span>';
            h += '</div>';
            return h;
        }
    },
    ...
    },
    {
        ...
        merge : true,
        template : function(item){
            return '<div class="ns-line-hiding" title="'+ ns.time_to_date(item.refund_action_time) +'">' + ns.time_to_date(item.refund_action_time) + '</div>';
        }
    },
    {
        title : "退款状态",
        ...
        merge : true,
        template : function(item){
            return '<div style="color: red">' + item.refund_status_name + '</div>';
        }
    },
    {
        title : "操作",
        ...
        merge : true,
        template : function(item){
            var html = '<div class="ns-table-btn"><a href="'+ ns.url("supply://admin/refund/detail",{order_goods_id:item.order_goods_id})+'" class="layui-btn"  target="_blank">查看详情</a></div>';
            return html;
