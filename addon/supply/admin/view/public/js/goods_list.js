...
$(function () {
    $("body").on("click", ".contraction", function () {
        ...
        if (open == 1) {
            ...
            $(this).children("span").text("-");
            $.ajax({
                url: ns.url("supply://admin/goods/getGoodsSkuList"),
...
function refreshTable(status) {
    ...
    var cols = [
        [{
            ...
            field: 'price',
            title: '<span style="padding-right: 15px;padding-top:10px;" class="ns-line-hiding" title="价格">价格</span>',
            align: 'center',
            unresize: 'false',
            width: '9%',
            templet: function (data) {
    ...
    table = new Table({
        elem: '#goods_list',
        url: ns.url("supply://admin/goods/lists"),
        cols: cols,
...
function verifyOn(goods_ids, verify_state) {
    if (verify_state === -2) {
        ...
        layer.prompt({
            ...
            $.ajax({
                url: ns.url("supply://admin/goods/verifyOn"),
                data: {
...
function lockup(goods_ids) {
    layer.prompt({
        ...
        yes: function (index, layero) {
            ...
            if (value) {
                $.ajax({
                    url: ns.url("supply://admin/goods/lockup"),
                    data: {
                        "verify_state_remark": value,
                        "goods_ids": goods_ids.toString()
                    },
...
function getVerifyStateRemark(goods_id, verify_state) {
    ...
    repeat_flag = true;
    $.ajax({
        url: ns.url("supply://admin/goods/getVerifyStateRemark"),
        data: {goods_id: goods_id},
        dataType: 'JSON',
        type: 'POST',
        success: function (res) {
...
function refreshVerifyStateCount() {
    $.ajax({
        url: ns.url("supply://admin/goods/refreshVerifyStateCount"),
        type: 'POST',
        dataType: 'JSON',
        success: function (res) {
