...
class MemberCancel extends BaseModel
{
    ...
    public function applyMemberCancel($param)
    {
        ...
        //判断是否申请过
        $member_cancel_info = model('member_cancel')->getInfo([['member_id', '=', $param['member_id']]]);
        ...
        if (addon_is_exit('fenxiao') == 1) {
            //获取分销商信息
            $fenxiao_info = model('fenxiao')->getInfo([['member_id', '=', $param['member_id']]]);
        ...
        try {
            ...
            $message_model->sendMessage(['keywords' => "USER_CANCEL_APPLY", 'member_id' => $member_info['member_id']]);
     ...
     * @return array
     */
    public function cancelApplyMemberCancel($member_id)
    {
        ...
        $res = model('member_cancel')->delete([['member_id', '=', $member_id]]);
    ...
    {
        $info = model('member_cancel')->getInfo([['id', '=', $param['id']]], 'member_id,status');
        if (empty($info)) {
        ...
        try {
            ...
            model('member_cancel')->update($data, [['id', '=', $param['id']]]);

            //删除会员信息
            model('member')->update(['is_delete' => 1,'status' => 0], [['member_id', '=', $info['member_id']]]);
            Cache::set('member_blacklist', null);

            event('MemberCancel', ['member_id' => $info['member_id']]);

            model('member_cancel')->commit();

            //会员注销成功消息
            $message_model = new Message();
            $message_model->sendMessage(['keywords' => "USER_CANCEL_SUCCESS", 'member_id' => $info['member_id']]);
    ...
    {
        $info = model('member_cancel')->getInfo([['id', '=', $param['id']]], 'status,member_id');
        ...
        $res = model('member_cancel')->update($data, [['id', '=', $param['id']]]);

        //会员注销失败消息
        $message_model = new Message();
        $message_model->sendMessage(['keywords' => "USER_CANCEL_FAIL", 'member_id' => $info['member_id']]);
     ...
     * @param $member_id
     */
    public function getMemberAccountInfo($member_id)
    {
        $data = [];
        //会员信息
        $member_info = model('member')->getInfo(
        ...
        //平台优惠券数量
        $data['member_platformcoupon_count'] = model('promotion_platformcoupon')->getCount(
            [
                ['member_id', '=', $member_id],
                ['state', '=', 1]
            ]
        ...
        //判断用户是否是分销商
        if ($member_info['is_fenxiao'] == 1 && addon_is_exit('fenxiao')) {
    ...
    public function memberCancelSuccess($data)
    {
        ...
        $member_info = model('member')->getInfo([ ['member_id', '=', $data['member_id']] ], 'username,mobile');
