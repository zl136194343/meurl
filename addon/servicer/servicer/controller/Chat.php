...
//        $read = $isClientOnline ? 1 : 0;
        ...
        if(!empty($this->uid)){
            $servicer = (new Servicer())->getInfo([['user_id', '=', $this->uid]], 'nickname,avatar');
            $dialogue['nickname'] = $servicer['data']['nickname'] ?? '';
            $dialogue['avatar'] = $servicer['data']['avatar'] ?? '';
        }
        $messageType = 'string';
        switch ( $contentType ) {
        ...
        $memberModel->createMember($member_id, $this->uid, 1, $client_id, $this->site_id);
    ...
    public function bindMember()
    {
        ...
        if ($isOnline) {
            ...
            if (empty($data)) {
                ...
                $memberModel->createMember($member_id, $this->uid, $isOnline, $client_ids[0], $this->site_id);
                // 向会员通知，有客服接入咨询

                // 客服信息
                $servicerModel = new Servicer();
                $servicer_info                = $servicerModel->handleServicerInfo([['user_id', '=', $this->uid]], $this->site_id);
                $servicer_info['servicer_id'] = $this->uid;
                $servicer_info['shop_id'] = $this->site_id;
                Gateway::sendToUid(
                    $member_id,
