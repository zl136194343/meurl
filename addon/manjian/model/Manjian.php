...
class Manjian extends BaseModel
{
    ...
    public function addManjian($data)
    {
        ...
        try {
            ...
            $cron = new Cron();
            if ($data[ 'start_time' ] <= time()) {
                $cron->addCron(1, 0, "满减送关闭", "CloseManjian", $data[ 'end_time' ], $manjian_id);
            } else {
                $cron->addCron(1, 0, "满减送开启", "OpenManjian", $data[ 'start_time' ], $manjian_id);
                $cron->addCron(1, 0, "满减送关闭", "CloseManjian", $data[ 'end_time' ], $manjian_id);
            }
        ...
        if (!empty($res)) {
            ...
            if (isset($res[ 'rule_json' ])) {
                ...
                foreach ($rule as $key => $item) {
                    ...
                    if (addon_is_exit('present', $site_id)) {
                        $present = $item[ 'present' ] ?? [];
                        if (!empty($present)) {
                            $present_model = new Present();
                            $present_info = $present_model->getPresentGoodsDetail([ [ 'pg.present_id', '=', $present[ 'present_id' ] ], [ 'pg.sku_id', '=', $present[ 'sku_id' ] ] ])[ 'data' ];
//                            $present_info = model('promotion_present')->getInfo([ ['present_id', '=', $present['present_id']] ], '*');
                            $present_info[ 'present_num' ] = $present[ 'present_num' ];
                            $rule[ $key ][ 'present_data' ] = $present_info;
                        }
                    }
                }
                $res[ 'rule' ] = $rule;
     ...
     * @param int $site_id
     */
    public function getGoodsManjianInfo($goods_id, $site_id = 0)
    {
        ...
        if ($site_id == 0) {
            $site_id = $goods_info['site_id'];
        }
        $condition[] = [ 'site_id', '=', $site_id ];
        $first_info = model('promotion_manjian')->getFirstData($condition, 'manjian_id,site_id,manjian_name,manjian_type,goods_ids,status,start_time,end_time,rule_json', 'create_time desc');
        if (!empty($first_info)) {
