...
layui.use(['form', 'laydate', 'laytpl'], function() {
	...
	form.on('submit(save)', function(data) {
		...
		$('.manjian-rule .level-item').each(function(i, e) {
			var limit = parseFloat($(this).find('.type-'+ type +' .layui-input').val());
			if (i > 0) {
			...
			if ($(this).find('[value="present"]').is(':checked')) {
				var present = $(this).find('.discount-item.present tr[data-present]').attr('data-present');
				var present_sku_id = $(this).find('.discount-item.present tr[data-present]').attr('data-present_sku_id');
				if (present == undefined) {
					showErrMsg('请选择要赠品');
					verify = false;
					return false;
				}
				var present_num = $(this).find('.discount-item.present tr[data-present]').find('input[name=present_num]').val();
				rule[limit].present = {present_id:present, present_num:present_num,sku_id:present_sku_id};
			}

			if (rule[limit].discount_money == undefined && rule[limit].free_shipping == undefined  && rule[limit].coupon == undefined && rule[limit].present == undefined) {
		...
		<div class="wrap">
			...
			<div class="content">
				...
				<div class="layui-input-block">
					...
					<div class="discount-item present">
                        <div>
                            <input type="checkbox" name="discount_type" value="present" class="ns-input-checkbox" lay-skin="primary"><span>送赠品</span>
                        </div>
                        <div class="discount-cont layui-hide">
                            <div><a href="javascript:;" class="ns-text-color select-present">选择赠品</a></div>
                            <div class="ns-word-aux">
                                <p>仅支持购买实物商品送赠品</p>
                            </div>
                            <div>
...
$('body').on('click', '.discount-item .select-coupon', function(e){
	...
	form.on('submit(coupon-search)', function(data) {
		...
		couponTable.reload({
			...
			where: {coupon_name:data.field.coupon_name}
		});
	});
...
$('body').on('click', '.discount-item .select-present', function(e){
	var event = this;
	...
	layer.open({
		...
		title: '赠品列表',
		skin: 'ns-layer-box',
		content: $("#presentList").html()
	});

	presentTable = new Table({
		elem: "#present_list",
		url: ns.url("present://shop/present/goodslists"),
		where: {'status': 2},
		cols: [
			[{
				title: '赠品名称',
				...
				width: '25%',
				templet: function (res) {
					return  ns.time_to_date(res.start_time) + "~" + ns.time_to_date(res.end_time);
				}
			}, {
				title: '操作',
				toolbar: '#presentOperation',
				unresize: 'false',
				align: 'center',
				width: '10%'
			}]
	...
	presentTable.tool(function(obj) {
		var data = obj.data;
		switch (obj.event) {
			case "add":
				addpresent(data);
	...
	function addpresent(data){
		var template = `<tr data-present="{{ d.present_id }}" data-present_sku_id="{{ d.sku_id }}">
			<td>{{ d.sku_name }}</td>
			<td><input type="number" name="present_num" value="1" lay-verify="present_num" placeholder="" autocomplete="off" class="layui-input ns-len-short"/></td>
	  		<td style="text-align:center;"><a href="javascript:;" onclick="deleteCoupon(this)" class="ns-text-color">删除</a></td>
  		</tr>`;
		laytpl(template).render(data, function(string){
			$(event).parents('.discount-cont').find('.layui-table tbody').html(string);
	 ...
	 * 搜索功能
	 */
	form.on('submit(present-search)', function(data) {

		presentTable.reload({
			page: {
				curr: 1
			},
			where: {goods_name:data.field.present_name}
		});
	});
});
