...
class Sms extends BaseModel
{
    ...
    public function createChildAccount($data)
    {
        ...
        if ($res[ 'code' ] == 0) {
            $config_model = new ConfigModel();
            $data[ 'signature' ] = "";
            $config_model->setSmsConfig($data, 1);
        }
        return $res;
    }
    ...
    public function loginAccount($data)
    {
        ...
        if ($res[ 'code' ] == 0 && !empty($res[ 'data' ])) {
            ...
            $config_model->setSmsConfig($res[ 'data' ], 1);
            // 刷新模板id
            $this->getSmsTemplatePageList([], 1, 0, 'template_id desc', 'template_id,tem_id,template_name,audit_status');
        }
        return $res;
    }
     ...
     * @return array|mixed
     */
    public function addChildSignature($data)
    {
    ...
    public function enableTemplate($template_id, $status, $sms_config)
    {
        ...
        try {
            ...
            if ($message_info) {
                ...
                $message_model->editMessage($message_data, [ [ 'keywords', '=', $template_info[ 'keywords' ] ] ]);
            } else {
                $message_data = [
