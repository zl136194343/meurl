...
class Sms extends BaseAdmin
{
    ...
    private $replace = [
        'NIU_SMS_CSS' => __ROOT__ . '/addon/niusms/admin/view/public/css',
        'NIU_SMS_JS' => __ROOT__ . '/addon/niusms/admin/view/public/js',
        'NIU_SMS_IMG' => __ROOT__ . '/addon/niusms/admin/view/public/img',
    ];
        ...
        if (!empty($sms_config[ 'value' ][ 'username' ])) {
            if ($return == 1) {
                ...
                if (!empty($account_info[ 'data' ])) {
                    ...
                    $signature_status = $sms_model->querySignature();
                    $this->assign("signature_status", $signature_status);

                    return $this->fetch("sms/index");
                } else {
                    $this->redirect(addon_url("niusms://admin/sms/login"));
                }
            }
        } else {
            // 未注册
...
//            }
    ...
    {
        $return_url = __ROOT__ . '/index.php/niusms/admin/sms/index?return=1';
        $sms_model = new SmsOrderModel();
    ...
    public function modifyConfigIsUse()
    {
        ...
        $result = $config_model->modifyConfigIsUse($is_use);
        return $result;
    }
    ...
    public function changeSignature()
    {
        if (request()->isAjax()) {
            ...
            //短信配置
            $config_model = new ConfigModel();
            $sms_config_result = $config_model->getSmsConfig();
            $sms_config = $sms_config_result[ 'data' ][ 'value' ];

            $sms_config[ 'signature' ] = $signature;

            $res = $config_model->setSmsConfig($sms_config, $sms_config_result[ 'data' ][ 'is_use' ]);
            return $res;
        }
    }
