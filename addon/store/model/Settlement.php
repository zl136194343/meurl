		...
		try {
            ...
            foreach ($store_list as $k => $store)
            {
                $online_settlement = model('order')->getInfo([
                    ...
                    'sum(order_money) as order_money, sum(refund_money) as refund_money,
                     sum(shop_money) as shop_money, sum(platform_money) as platform_money, 
                     sum(refund_shop_money) as refund_shop_money, sum(refund_platform_money) as refund_platform_money, 
                     sum(commission) as commission, sum(platform_coupon_money) as platform_coupon_money, sum(refund_platform_coupon_money) as refund_platform_coupon_money');
                ...
                $offline_settlement = model('order')->getInfo([
                    ...
                    ['store_settlement_id', '=', 0],
                    ['pay_type', '=', 'OFFLINE_PAY']
                ], 'sum(order_money) as offline_order_money, sum(refund_money) as offline_refund_money, sum(platform_coupon_money) as offline_platform_coupon_money, sum(refund_platform_coupon_money) as offline_refund_platform_coupon_money');
                ...
                $settlement = [
                    ...
                    'platform_coupon_money' => $online_settlement['platform_coupon_money'] ?? 0,
                    'refund_platform_coupon_money' => $online_settlement['refund_platform_coupon_money'] ?? 0,
                ];
