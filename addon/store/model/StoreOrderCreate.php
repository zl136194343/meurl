...
//        Cache::tag("order_create_store_".$data['member_id'])->set("order_create_seckill_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
    ...
    public function getStoreGoodsList($sku_ids, $nums, $data)
    {
        ...
        $goods_list = model("store_goods_sku")->getList([['sgs.sku_id', 'in', $sku_ids], ['sgs.store_id', '=', $data['store_id']],['ngs.is_delete','=',0]], $field, '', $alias, $join);

//            $shop_goods_list = [];
        if (!empty($goods_list)) {
            foreach ($goods_list as $k => $v) {

                $goods_info = model('goods')->getInfo([['goods_id', '=', $v['goods_id'],['is_delete','=',0]]], 'commission_rate,goods_image');
