			...
			goodsSources: [
				{
					text: "默认",
					value: "default",
					src: couponResourcePath + "/admin_coupon/img/goods.png",
					selectedSrc: couponResourcePath + "/admin_coupon/img/goods_1"+url+".png"
				},
				{
					text : "手动选择",
					value : "diy",
					src: couponResourcePath + "/admin_coupon/img/manual.png",
					selectedSrc: couponResourcePath + "/admin_coupon/img/manual_1"+url+".png"
				}
			],
			isLoad: false,
			isShow: false,
	...
	methods: {
		...
		addCoupon: function(){
			...
			self.couponSelect(function (res) {
				self.$parent.data.couponIds = res.toString();
			}, self.$parent.data.couponIds);
		},
		couponSelect: function(callback, selectId) {
			var self = this;
			layui.use(['layer'], function () {
				var url = ns.url("platformcoupon://admin/platformcoupon/couponselect", {select_id : selectId.toString()});
				//iframe层-父子操作
				layer.open({
					title: "优惠券选择",
					type: 2,
