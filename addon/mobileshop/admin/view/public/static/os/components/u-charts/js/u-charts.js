...
var config = {
	...
	colors: ['#1890ff', '#2fc25b', '#facc14', '#f04864', '#8543e0', '#90ed7d', '#5caf20', '#30b0d7', '#ff6d58', '#1A79B9',
		'#f97723', '#f3792c'
	],
...
//设置内容
function getToolTipData(seriesData, calPoints, index, categories, opts) {
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
	...
	var toolTipHeight = 2 * config.toolTipPadding + (textList.length + 1) * config.toolTipLineHeight;
	...
	if (textList.length > 0) {
		...
		context.setFontSize(config.fontSize);
		context.setFillStyle(toolTipOption.fontColor);
		context.fillText(textList[0].time, startX, startY - config.fontSize);
		context.closePath();
		context.stroke();
	}
...
function drawAreaDataPoints(series, opts, config, context) {
	...
	series.forEach(function(eachSeries, seriesIndex) {
		...
		for (let i = 0; i < splitPointList.length; i++) {
			...
			grad.addColorStop(0, hexToRgb(eachSeries.color, areaOption.opacity)); //定义渐变色颜色
			grad.addColorStop(1, hexToRgb(eachSeries.color, 0));
			...
			context.closePath();
			context.fill();
			context.strokeStyle = opts.series[seriesIndex].color ? opts.series[seriesIndex].color : 'blue'
			//画连线
			...
			if (points.length === 1) {
				...
				if (lineOption.type === 'curve') {
					points.forEach(function(item, index) {
						if (index > 0) {
							var ctrlPoint = createCurveControlPoints(points, index - 1);
							context.lineTo(ctrlPoint.ctrA.x, ctrlPoint.ctrA.y);
							context.bezierCurveTo(ctrlPoint.ctrA.x, ctrlPoint.ctrA.y, ctrlPoint.ctrB.x, ctrlPoint.ctrB.y, item.x, item
								.y);
						}
					});
...
function drawYAxisGrid(categories, opts, config, context) {
	...
	context.beginPath();
	context.setStrokeStyle(opts.yAxis.gridColors || "#cccccc");
	context.setLineWidth(1 * opts.pixelRatio);
	points.forEach(function(item, index) {
		...
		for (let i = 0; i < itemList.length; i++) {
			...
			switch (opts.type) {
				case 'line':
					...
					context.setFillStyle(item.color);
					//划线
					context.moveTo(startX + 7.5 * opts.pixelRatio - 10, startY + 5 * opts.pixelRatio);
					context.lineTo(startX + 7.5 * opts.pixelRatio + 10, startY + 5 * opts.pixelRatio);
					// context.moveTo(startX + 7.5 * opts.pixelRatio, startY + 5 * opts.pixelRatio);

					context.arc(startX + 7.5 * opts.pixelRatio, startY + 5 * opts.pixelRatio, 3 * opts.pixelRatio, 0, 2 * Math.PI);
					context.closePath();
					context.fill();
					context.stroke();
					break;
...
var Charts = function Charts(opts) {
	...
	if (opts.type == 'pie' || opts.type == 'ring') {
		config$$1.pieChartLinePadding = opts.dataLabel === false ? 0 : opts.extra.pie.lableWidth * opts.pixelRatio ||
...
Charts.prototype.updateData = function() {
	...
	this.opts.yAxis.format = data.unit ? ((val) => {
		return val.toFixed(data.fixed || 0) + (data.unit || '')
	}) : this.opts.yAxis.format;
	...
	if (this.opts.type === 'line' || this.opts.type === 'area' || this.opts.type === 'column') {
		...
		if (index > -1) {
			...
			if (seriesData.length !== 0) {
				var _getToolTipData = getToolTipData(seriesData, this.chartData.calPoints, index, this.opts.categories, this.opts),
