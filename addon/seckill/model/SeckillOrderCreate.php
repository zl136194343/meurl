...
class SeckillOrderCreate extends OrderCreate
{
    ...
    public function create($data)
    {
        ...
        try {
            ...
            foreach ($shop_goods_list['goods_list'] as $k_order_goods => $order_goods) {
                ...
                model("promotion_seckill_goods")->setDec([['sku_id', '=', $order_goods['sku_id']], ['seckill_id', '=', $calculate_data["seckill_info"]["id"]]], "stock", $order_goods['num']);
            ...
            //订单完成后事件
            $this->orderCreateAfter($order_id);
            model("order")->commit();
    ...
    public function calculate($data)
    {
        ...
        $shop_goods_list['promotion_id'] = $seckill_id;
        $seckill_info_result = $seckill_model->getSeckillInfo($seckill_id);
        ...
        //秒杀库存
        if ($shop_goods_list["goods_list"][0]) {
            $seckill_goods = $seckill_model->getSeckillGoodsInfo([['psg.sku_id', '=', $shop_goods_list["goods_list"][0]['sku_id']]], 'psg.stock');
            $seckill_goods_stock = $seckill_goods['data']['stock'];
            if ($shop_goods_list["goods_list"][0]['num'] > $seckill_goods_stock) {
                $this->error = 1;
                $this->error_msg = "当前秒杀活动商品库存不足";
            }
        }
        // 秒杀商品限购 按每日某时段限购
        if ($shop_goods_list["goods_list"][0]['limit_num'] > 0) {
            $purchased_num = $this->getGoodsPurchasedNum($shop_goods_list["goods_list"][0]['sku_id'], $data['member_id'], $seckill_info['id']);
            if (($purchased_num + $shop_goods_list["goods_list"][0]['num']) > $shop_goods_list["goods_list"][0]['limit_num']) {
                $this->error = 1;
                $this->error_msg = "该商品每人限购{$shop_goods_list[ "goods_list" ][ 0 ]['limit_num']}件，您已购买{$purchased_num}件";
            }
        }
    ...
    public function CronOrderClose($order_id)
    {
        ...
        if (!empty($order_info)) {
            ...
            $order_goods_list = model('order_goods')->getList($condition, "order_goods_id,sku_id,num,refund_status");
            $order_refund_model = new OrderRefund();
            foreach ($order_goods_list as $k => $v) {
                if ($v["refund_status"] != $order_refund_model::REFUND_COMPLETE) {
                    $condition = array(
                        "sku_id" => $v["sku_id"],
                        "seckill_id" => $order_info["promotion_id"],
                    );
                    model("promotion_seckill_goods")->setInc($condition, "stock", $v["num"]);
                }
            }
        }
        return $this->success();
