...
class Poster extends BaseModel
{
    ...
    public function goods($app_type, $page, $qrcode_param, $promotion_type = 'null'){
        try {
            ...
            $qrcode_param['seckill_id'] = $goods_info['seckill_id'];
            unset($qrcode_param['id']);
            ...
            $res = $option_res->jpeg('upload/poster/goods', 'goods_' . $promotion_type . '_' . $qrcode_param['seckill_id'] . '_' . $qrcode_param['source_member'] . '_' . $app_type);
            return $res;
        } catch (\Exception $e) {
    ...
    private function getGoodsInfo($id){
        ...
        $field = 'ngs.sku_name,ngs.introduction,ngs.sku_image,ngs.sku_id,npsg.seckill_price, npsg.seckill_id';
    ...
    private function getGoodsQrcode($app_type, $page, $qrcode_param, $promotion_type = 'null'){
        $res = event('Qrcode', [
            ...
            'qrcode_name' => 'goods_' . $promotion_type . '_' . $qrcode_param['seckill_id'] . '_' . $qrcode_param['source_member'],
        ], true);
        return $res;
    }
}