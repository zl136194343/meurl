...
class Replay extends BaseModel
{
     ...
     */
    public function getSiteWechatKeywordsReplay($keywords)
    {
        $list = model('wechat_replay_rule')->getList([['rule_type' ,'=', 'KEYWORDS' ]]);
        $rule_list = array();
        $text = '';
        foreach ($list as $k => $v) {
     ...
     * @param unknown $keywords
     */
    function getWechatFollowReplay()
    {
        $follow_info = model('wechat_replay_rule')->getInfo([['rule_type','=', 'AFTER' ]]);
        $replay_content = '';
