...
WxReplay.prototype.getData = function (d) {
	...
	_this.sendAjax({
		url: ns.url('wechat://admin/replay/follow'),
		async: false,
	...
	delRule: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: ns.url('wechat://admin/replay/deleterule'),
	...
	delKeywords: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: ns.url('wechat://admin/replay/deletekeywords'),
	...
	delReply: function (d) {
		...
		var index = layer.open({
			...
			, yes: function (index, layero) {
				...
				flag = false;
				_this.sendAjax({
					url: ns.url('wechat://admin/replay/deleteReply'),
					data: {"rule_id": rule_id, "key_id": key_id},
					success: function (res) {
						layer.msg(res.message);
						if (res.code >= 0) {
