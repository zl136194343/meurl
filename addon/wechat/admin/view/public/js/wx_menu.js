...
var menu = new Vue({
	...
	methods: {
		...
		addMaterial: function (type) {
			switch (type) {
				case 1 :
					window.open(ns.url("wechat://admin/material/add"));
					break;
				case 2 :
					uploadSingle();
					break;
		...
		loadMenu: function () {
			var _self = this;
			$.ajax({
				url: ns.url('wechat://admin/menu/menu'),
				data: {},
				dataType: "JSON",
		...
		saveMenu: function () {
			...
			$.ajax({
				url: ns.url("wechat://admin/menu/edit"),
				data: {value:value, json_data:json_data},
				dataType: "JSON",
				success: function (res) {
					layer.msg(res.message);
				},
...
function singleImageUploadSuccess(_data, _name) {
	if (_data.path != undefined) {
		...
		$.ajax({
			type: 'post',
			url: ns.url('wechat://admin/material/add'),
			dataType: "JSON",
			data: {
				'type': 2,
				'path': _data.path,
