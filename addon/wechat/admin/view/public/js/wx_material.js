...
function loadMaterialList(type) {
	var table = new Table({
		...
		url: ns.url('wechat://admin/material/lists'),
		where: {type: 1},
		cols: [[
			...
			{title: '　　操作', width: '15%', toolbar: '#action',unresize : 'true', align: 'left'}
		]]
	});
	...
	table.tool(function (obj) {
		...
		switch (obj.event) {
			case "edit":
				window.location.href = ns.url('wechat://admin/material/edit', {"id": data.id});
				break;
			case "delete":
...
function preview(id, index = 0) {
	var url = ns.url('wechat://admin/material/previewgraphicmessage', {"id": id, "i": index});
	window.open(url);
}
...
function delMaterial(id, type, media_id) {
	layer.confirm(
		...
		function (index, layero) {
			$.ajax({
				url: ns.url('wechat://admin/material/delete'),
