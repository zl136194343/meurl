	...
	if (type == 1) {
		var table = new Table({
			...
			cols: [[
				...
				{title: '操作', width: '20%', toolbar: '#action', align: 'center'}
			]]
		});
...
function preview(id, index = 0) {
	...
	var url = ns.url("wechat://admin/material/previewgraphicmessage");
	...
	if (type == 5) {
		layui.use('form', function () {
			...
			form.on('submit(addText)', function (data) {
				...
				$.ajax({
					type: 'post',
					url: ns.url('wechat://admin/material/addTextMaterial'),
					data: {type: 5, value:value},
					dataType: "JSON",
