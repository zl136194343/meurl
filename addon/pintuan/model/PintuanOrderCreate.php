 ...
 *
 */
class PintuanOrderCreate extends OrderCreate
{
    ...
    public function create($data)
    {
        ...
        try {
            ...
            $data_order = [
                ...
                "promotion_type_name" => "拼团",
                "promotion_status_name" => "待参团",
            ...
            foreach ($shop_goods_list[ 'goods_list' ] as $k_order_goods => $order_goods) {
                ...
                $result = $pintuan_order_model->addPintuanOrder($data_order, $calculate_data[ "group_id" ], $calculate_data[ "pintuan_info" ][ "pintuan_id" ]);
                if ($result[ "code" ] != 0) {
                    model("order")->rollback();
                    return $result;
                }
            ...
            Cache::tag("order_create_pintuan_" . $data[ 'member_id' ])->clear();
    ...
    public function calculate($data)
    {
        ...
        $pintuan_id = $shop_goods_list[ "goods_list" ][ 0 ][ "pintuan_id" ];

        $pintuan_info_result = $pintuan_model->getPintuanInfo([ [ "pintuan_id", "=", $pintuan_id ] ]);
        $pintuan_info = $pintuan_info_result[ "data" ];
        if (empty($pintuan_info)) {
            return $this->error([], "找不到有效的拼团活动!");
        }
        //判断购买数是否超过限购
        if ($pintuan_info[ "buy_num" ] < $data[ 'num' ] && $pintuan_info[ "buy_num" ] > 0) {
            $this->error = 1;
            $this->error_msg = "该商品限制购买不能多于" . $pintuan_info[ "buy_num" ] . "件!";
        }

        //判断是否可参团
        $pintuan_order = new PintuanOrder();
        $result = $pintuan_order->isCanJoinGroup($data[ 'group_id' ], $data[ 'member_id' ]);
        if ($result[ 'code' ] < 0) {
            $this->error = 1;
            $this->error_msg = $result[ 'message' ];
        }

        $data[ "pintuan_info" ] = $pintuan_info;
        //查询拼团组信息
        $pintuan_model = new PintuanGroup();
        $group_id = $data[ "group_id" ];
        $pintuan_group_info_result = $pintuan_model->getPintuanGroupInfo([ [ "group_id", "=", $group_id ] ]);

        $pintuan_group_info = $pintuan_group_info_result[ "data" ];
        $data[ "pintuan_group_info" ] = $pintuan_group_info;

        $shop_goods_list['promotion_id'] = $pintuan_id;
    ...
    public function getOrderGoodsCalculate($data)
    {
        $shop_goods_list = [];
        //通过拼团id查询 商品数据
//        $cache = Cache::get("order_create_pintuan_".$data['id'].'_'.$data['num'].'_'.$data['member_id']);
//        if(!empty($cache))
//        {
//            return $cache;
//        }
        $goods_list = $this->getPintuanGoodsInfo($data[ "id" ], $data);
        $goods_list[ 'promotion_money' ] = 0;
        $shop_goods_list = $goods_list;
//        Cache::tag("order_create_pintuan_".$data['member_id'])->set("order_create_pintuan_".$data['id'].'_'.$data['num'].'_'.$data['member_id'], $shop_goods_list, 600);
     ...
     * @param unknown $bl_id
     */
    public function getPintuanGoodsInfo($id, $data)
    {
        //组装商品列表
        ...
        $info = model("promotion_pintuan_goods")->getInfo([ [ 'nppg.id', '=', $id ],['ngs.is_delete','=',0] ], $field, $alias, $join);
        $shop_goods_list = [];

        if (!empty($info)) {
            //用于过滤商品
            if ($info[ 'goods_state' ] != 1 || $info[ 'verify_state' ] != 1) {
                $this->error = 1;
            ...
            $goods_info = model('goods')->getInfo([ [ 'goods_id', '=', $info[ 'goods_id' ] ] ], 'commission_rate,goods_image');
            $info[ 'commission_rate' ] = $goods_info[ 'commission_rate' ] ?? 0;
            $info[ 'goods_image' ] = $goods_info[ 'goods_image' ] ?? '';
            $info[ "num" ] = $data[ "num" ];
            //如果数量小于0,就提示报错
            if($info[ 'num' ] <= 0){
                $this->error = 1;
