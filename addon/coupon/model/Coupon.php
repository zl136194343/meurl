...
class Coupon extends BaseModel
{
     ...
     */
    public function receiveCoupon($coupon_type_id, $member_id, $get_type, $is_stock = 0, $is_limit = 1)
    {
        ...
        $coupon_type_info = model('promotion_coupon_type')->getInfo([ [ 'coupon_type_id', '=', $coupon_type_id ] ]);
        if (!empty($coupon_type_info)) {
            ...
            $data = [
                'coupon_type_id' => $coupon_type_id,
                'site_id' => $coupon_type_info[ 'site_id' ],
                'site_name' => $coupon_type_info[ 'site_name' ],
    ...
    public function getMemberCouponPageList($condition, $page = 1, $page_size = PAGE_LIST_ROWS)
    {
		...
		npc.at_least,npc.money,npc.discount,npc.state,npc.get_type,npc.fetch_time,npc.use_time,npc.end_time,mem.username,on.order_no,npc.site_name,npc.goods_type';
        $alias = 'npc';
        $join = [
            [
                'member mem',
     ...
     * @return array
     */
    public function getCouponTypeList($condition = [], $field = true, $order = '', $alias = '', $join = [], $group = '', $limit = null)
    {
        $list = model("promotion_coupon_type")->getList($condition, $field, $order, $alias, $join, $group, $limit);
        return $this->success($list);
    }
