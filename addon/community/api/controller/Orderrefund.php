...
class Orderrefund extends BaseApi
{
    ...
    public function lists()
    {
        ...
        if(!empty($this->params['cl_id'])) {
            $condition[] = ["no.cl_id", "=", $this->params['cl_id']];
        }else{
            $condition[] = ["nop.member_id", "=", $this->member_id??30];
        }
...
//                break;
    ...
    public function refund()
    {
        ...
        $member_info_result = $member_model->getMemberInfo([["member_id", "=", $this->member_id??30]]);
    ...
    public function detail()
    {
        ...
        $order_refund_model      = new OrderRefundModel();
        $order_goods_id          = isset($this->params['order_goods_id']) ? $this->params['order_goods_id'] : '0';
        $order_goods_info_result = $order_refund_model->getMemberRefundDetail($order_goods_id, $this->member_id??30);
        if ($order_goods_info_result["data"]) {
            //查询店铺收货地址
            $shop_model                                      = new ShopModel();
            $shop_info_result                                = $shop_model->getShopInfo([["site_id", "=", 0]], "full_address,address");
