...
class BaseApi
{
    ...
    public function __construct()
    {
        ...
        $this->params = input();
        $this->params['app_type'] = empty($this->params['app_type']) ?$this->params['app_type'] :"weapp";
        $this->params['app_type'] = empty($this->params['app_type_name']) ?$this->params['app_type_name'] :"微信小程序";
        $this->params['web_city'] = empty($this->params['web_city']) ?$this->params['web_city'] :0;
    ...
    private function getLang()
    {
        ...
        if (empty($cache_common)) {
            $cache_common = include 'app/api/lang/' . $default_lang . 'Virtualgoods.php';
        ...
        if (!empty($addon)) {
            try {
                ...
                if (!empty($addon_cache_common)) {
                    ...
                    $addon_cache_common = include 'addon/' . $addon . '/api/lang/' . $default_lang . 'Virtualgoods.php';
                    if (!empty($addon_cache_common)) {
                        $cache_common = array_merge($cache_common, $addon_cache_common);
        ...
        if (!empty($lang_path)) {
            ...
            if (empty($cache_path)) {
                $cache_path = include $lang_path . "/" . $default_lang . 'Virtualgoods.php';
