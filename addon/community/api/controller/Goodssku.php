...
class Goodssku extends BaseApi
{
    ...
    public function detail()
    {
        ...
        $goods = new Goods();
        $goods_sku_detail = $goods->getGoodsSkuDetail($sku_id, $this->site_id??0);
...
/*        $goods_service = new GoodsService();
        ...
        if ($token[ 'code' ] >= 0) {
            if (!empty($list[ 'data' ][ 'list' ])) {
                foreach ($list[ 'data' ][ 'list' ] as $k => $v) {
                    ...
                    $list['data']['list'][$k]['activity_type'] = '';
                    $list['data']['list'][$k]['activity_id'] = '';
                }
            }
        }
    ...
    public function recommend()
    {
        ...
        (g.sale_num + g.virtual_sale) as sale_num,gs.sku_image,gs.goods_name,gs.site_id,gs.is_free_shipping,
        gs.introduction,gs.promotion_type,g.goods_image,gs.unit,g.goods_spec_format';
        $alias = 'gs';
        $join = [
            [ 'goods g', 'gs.sku_id = g.sku_id', 'inner' ]
        ];
