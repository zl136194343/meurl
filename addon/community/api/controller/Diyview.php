...
class Diyview extends BaseApi
{
    ...
    public function memberIndex()
    {
        ...
        if ($token['code'] == 0) {
            $delivery_clerk_id = model('member')->getValue([['member_id', '=', $this->member_id]], 'delivery_clerk_id');
            $cl_id = model('member')->getValue([['member_id', '=', $this->member_id]], 'cl_id');
        }
        ...
        $community_config = $config_model->getConfig($this->site_id);
        ...
        foreach ($info['menuList'] as $k => $v) {
            ...
            // 判断配送员菜单，是否显示
            if ($v['link']['name'] == 'DELIVERY' && empty($delivery_clerk_id)) {
                unset($info['menuList'][$k]);
                continue;
            }
            // 判断团长菜单，是否显示
            if ($v['link']['name'] == 'COMMUNITYCENTER') {
                if (empty($community_config['data']['value']['is_allow_apply']) && empty($cl_id)) {
                    unset($info['menuList'][$k]);
                    continue;
                }
            }
