...
class Config extends BaseModel
{
    ...
    public function getDeliverTypeSort($site_id)
    {
        ...
        if (empty($res['data']['value'])) {
            $res['data']['value'] = ['deliver_type' => 'community'];
        }
        return $res;
    }
     ...
     * @return array
     */
    public function setCommunityDeliveryConfig($data, $is_use, $site_id)
    {
        if ($site_id === '') {
            return $this->error('', '缺少必须参数站点id');
        }
        $config = new ConfigModel();
        $res    = $config->setConfig($data, '社区自提配置', $is_use, [['site_id', '=', $site_id], ['app_module', '=', 'shop'], ['config_key', '=', 'EXPRESS_COMMUNITY_DELIVERY_CONFIG']]);
        return $res;
    }
