...
class Withdraw extends BaseModel
{
     ...
     * @return array
     */
    public function addCommunityWithdraw($data)
    {
        //获取团长信息
        $fenxiao_model = new Leader();
        $fenxiao_info = $fenxiao_model->getLeaderInfo([ [ 'cl_id', '=', $data[ 'cl_id' ] ] ], 'cl_id,name,commission_total');
        if ($fenxiao_info[ 'data' ][ 'commission_total' ] < $data[ 'money' ]) {
        ...
        try {
            ...
            $data[ 'commander_id' ] = $fenxiao_info[ 'data' ][ 'cl_id' ];
            $data[ 'commander_name' ] = $fenxiao_info[ 'data' ][ 'name' ];

            $res = model('commander_withdraw')->add($data);
            ...
            //修改分销商提现中金额
            model('community_leader')->setInc([ [ 'member_id', '=', $data[ 'member_id' ] ] ], 'account_withdraw_apply', $data[ 'money' ]);
            //修改分销商可提现金额
            model('community_leader')->setDec([ [ 'member_id', '=', $data[ 'member_id' ] ] ], 'commission_total', $data[ 'money' ]);

            model('commander_withdraw')->commit();

            //申请提现发送消息
            /*$data[ 'keywords' ] = 'COMMANDER_WITHDRAWAL_APPLY';
    ...
    public function withdrawPass($ids)
    {
        ...
        try {
            ...
            foreach ($withdraw_list[ 'data' ] as $k => $v) {
                ...
                //修改团长提现中金额
                model('community_leader')->setDec([ [ 'cl_id', '=', $v[ 'commander_id' ] ] ], 'account_withdraw_apply', $v[ 'money' ]);
                //修改分销商已提现金额
                model('community_leader')->setInc([ [ 'cl_id', '=', $v[ 'commander_id' ] ] ], 'account_withdraw', $v[ 'money' ]);

                //添加会员账户流水
                $member_account = new MemberAccount();
                $member_result = $member_account->addMemberAccount($v[ 'member_id' ], 'withdraw_money', $v[ 'real_money' ], 'leader', '佣金提现', '团长佣金提现');
...
/*                $message_model = new Message();
     ...
     * @return array
     */
    public function withdrawRefuse($id,$remark)
    {
        $data = [
            'status' => -1,
            'payment_time' => time(),
        ...
        try {
            ...
            //修改分销商提现中金额
            model('community_leader')->setDec([ [ 'cl_id', '=', $info[ 'commander_id' ]] ], 'account_withdraw_apply', $info[ 'money' ]);
            //修改分销商可提现金额
            model('community_leader')->setInc([ [ 'cl_id', '=', $info[ 'commander_id' ] ] ], 'commission_total', $info['money']);
...
/*            $message_model = new Message();
    ...
    {
        $res = model('commander_withdraw')->getInfo($condition, $field);
        return $this->success($res);
    }
    ...
    {
        $res = model('commander_withdraw')->getCount($condition, $field);
        return $this->success($res);
    }
}