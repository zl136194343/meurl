...
class OrderRefund extends BaseModel
{
    ...
    {
        //订单商品项
        $order_goods_info = model("commander_order_goods")->getInfo([
            'order_goods_id' => $order_goods_id
        ]);
        $count = model("commander_order_goods")->getCount([ [ 'order_id', '=', $order_goods_info[ 'order_id' ] ], [ 'refund_status', '=', 0 ], [ 'order_goods_id', '<>', $order_goods_id ] ], 'order_goods_id');
        if ($count > 0) {
            $delivery_money = 0;
            $invoice_delivery_money = 0;
            $invoice_money = 0;
        } else {
            $delivery_num = model("commander_order_goods")->getCount([ [ 'order_id', '=', $order_goods_info[ 'order_id' ] ], ['delivery_status', '=', 1] ]);
            //订单整体项
            $order_info = model("commander_order")->getInfo([ 'order_id' => $order_goods_info[ 'order_id' ]
    ...
    public function getOrderRefundBalance($order_goods_id)
    {
        ...
        //订单整体项
        $order_info = model("commander_order")->getInfo([
    ...
    public function addOrderRefundLog($order_goods_id, $refund_status, $action, $action_way, $action_userid, $action_username, $desc = '')
    {
        ...
        $res = model('commander_order_refund_log')->add($data);
        return $res;
    }
    ...
    {
        $order_goods_info = model("commander_order_goods")->getInfo([ 'order_goods_id' => $data[ 'order_goods_id' ] ], 'order_id,refund_status,delivery_status,is_virtual,site_id');
        ...
        model("commander_order_goods")->startTrans();
        ...
        try {
            ...
            $res = model("commander_order_goods")->update($data, [ 'order_goods_id' => $data[ 'order_goods_id' ] ]);
            ...
            event('CommunityOrderRefundApply', $data);//传入订单类型以及订单项id
            model("commander_order_goods")->commit();
            ...
            model("commander_order_goods")->rollback();
    ...
    {
        $order_goods_info = model("commander_order_goods")->getInfo([ 'order_goods_id' => $data[ 'order_goods_id' ] ]);
        if (empty($order_goods_info)) {
            return $this->error();
        }
        model("commander_order_goods")->startTrans();
...
//            $order_info = model("commander_order")->getInfo(['order_id' => $order_goods_info['order_id']]);
    ...
    public function orderRefundRefuse($data, $user_info, $refund_refuse_reason)
    {
        ...
        try {
            ...
            event("CommunityOrderRefundRefuse", $data);
            model("commander_order_goods")->commit();
...
/*            $shop_info_result = $shop_model->getShopInfo([ [ "site_id", "=", $order_goods_info[ "site_id" ] ] ], "full_address");
    ...
    public function orderRefundFinish($data, $user_info)
    {
        ...
        try {
            ...
            $res = model("commander_order_goods")->update($update_data, [ [ 'order_goods_id', "=", $data[ 'order_goods_id' ] ] ]);
            ...
            event('CommunityOrderRefundFinish', $order_goods_info);//传入订单类型以及订单项id
            model("commander_order_goods")->commit();
            return $this->success();
        } catch (\Exception $e) {
            model("commander_order_goods")->rollback();
    ...
    {
        $order_goods_info = model("commander_order_goods")->getInfo(['order_goods_id' => $order_goods_id]);
        ...
        try {
            ...
            //验证订单是否全部退款完毕
            $order_goods_count = model("commander_order_goods")->getCount([ [ "order_id", "=", $order_goods_info[ 'order_id' ] ] ], "order_goods_id");
            $refund_count = model("commander_order_goods")->getCount([ [ "order_id", "=", $order_goods_info[ 'order_id' ] ], [ "refund_status", "=", self::REFUND_COMPLETE ] ], "order_goods_id");
            $refund_total_real_money = model("commander_order_goods")->getSum([ [ "order_id", "=", $order_goods_info[ 'order_id' ] ], [ "refund_status", "=", self::REFUND_COMPLETE ] ], "refund_real_money");
...
//                    $refunded_balance_money = model("commander_order_goods")->getSum([["order_id", "=", $order_goods_info['order_id']], ["refund_status", "=", self::REFUND_COMPLETE]], "order_goods_id");
//                    $refunded_balance_money = model("commander_order_goods")->getSum([["order_id", "=", $order_goods_info['order_id']], ["refund_status", "=", self::REFUND_COMPLETE]], "order_goods_id");
...
//                }
            ...
            //订单累加 退款
            $change_data = ['refund_money' => $refund_total_real_money];
            if($order_info['order_type'] == 5){
                ...
                if($config['commission_type'] == 1){
                    $commission = round(($order_info['order_money'] - $refund_total_real_money) * $header['commission_rate'] * 0.01, 2);
                }else{
                    $commission = $header['commission_money'];
                }
                $change_data['commission'] = $commission;
            }
            model("commander_order")->update($change_data, [ [ "order_id", "=", $order_goods_info[ 'order_id' ] ] ]);
            event('CommunityOrderRefundFinish', $order_goods_info);//传入订单类型以及订单项id
            model("commander_order_goods")->commit();
            return $this->success();
        } catch (\Exception $e) {
    ...
    {
        //验证订单是否全部退款完毕  订单如果全部退款完毕,订单关闭
        $order_goods_count = model("commander_order_goods")->getCount([ [ "order_id", "=", $order_id ] ], "order_goods_id");
        $refund_count = model("commander_order_goods")->getCount([ [ "order_id", "=", $order_id ], [ "refund_status", "=", self::REFUND_COMPLETE ] ], "order_goods_id");
        if ($order_goods_count == $refund_count) {
            //调用订单关闭
            $order_common_model = new OrderCommon();
    ...
    {
        $list = model('commander_order_refund_log')->getList($condition, $field, $order, '', '', '', $limit);
    ...
    public function getRefundOrderGoodsPageList($condition = [], $page = 1, $page_size = PAGE_LIST_ROWS, $order = '', $field = 'nop.*,no.order_no,no.site_id,no.site_name')
    {
        ...
        $list = model("commander_order_goods")->pageList($condition, $field, $order, $page, $page_size, 'nop', $join);
        if (!empty($list[ "list" ])) {
    ...
    {
        $count = model("commander_order_goods")->getCount($condition);
    ...
    public function initOrderGoodsRefundAction($condition)
    {
        ...
        $result = model("commander_order_goods")->update($data, $condition);
    ...
    public function getMemberRefundDetail($order_goods_id, $member_id)
    {
        ...
        $info = model("commander_order_goods")->getInfo($condition);
        ...
        //将售后日志引入
        $refund_log_list = model("commander_order_refund_log")->getList([ [ "order_goods_id", "=", $order_goods_id ] ], "*", "action_time desc");
    ...
    {
        $order_info = model("commander_order")->getInfo([ [ "order_id", "=", $order_id ] ], "pay_money, out_trade_no, site_id, delivery_money");
...
//            $refund_result = $pay_model->refund($refund_no, $order_info["pay_money"], $order_info["out_trade_no"], '', $order_info["pay_money"], $order_info["site_id"], 1);
    ...
    public function activeOrderGoodsRefund($order_goods_id, $refund_money, $remark = '', $refund_reason = '')
    {
        ...
        try {
            ...
            $update_data[ 'refund_status_name' ] = $this->order_refund_status[ self::REFUND_COMPLETE ][ "name" ];
            $update_data[ 'refund_status_action' ] = json_encode($this->order_refund_status[ self::REFUND_COMPLETE ], JSON_UNESCAPED_UNICODE);
            $res = model("commander_order_goods")->update($update_data, [ [ 'order_goods_id', "=", $order_goods_id ] ]);
            if ($res === false) {
                model("commander_order_goods")->rollback();
                return $this->error();
            }
    ...
    public function verifyOrderLock($order_id)
    {
        ...
        $count = model("commander_order_goods")->getCount($condition, "order_goods_id");
    ...
    public function orderRefundClose($order_goods_id, $site_id, $user_info){
        $order_goods_info = model("commander_order_goods")->getInfo([ 'order_goods_id' => $order_goods_id, 'site_id' => $site_id ]);
        if (empty($order_goods_info)) {
            return $this->error();
        }
        model("commander_order_goods")->startTrans();
        try {
            ...
            $res = model("commander_order_goods")->update($data, [ 'order_goods_id' => $order_goods_id ]);
