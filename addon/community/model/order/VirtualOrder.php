...
use app\model\community\VirtualGoods;
...
class VirtualOrder extends OrderCommon
{
    ...
    public function orderPay($order_info, $pay_type)
    {
        ...
        $res           = model('commander_order')->update($data, [['order_id', "=", $order_info['order_id']]]);
    ...
    {
        $order_info = model('commander_order')->getInfo([['order_id', '=', $order_id]], 'order_no,site_id,site_name,member_id,order_type,
        ...
        try {
            //订单项变为已发货
            model('commander_order_goods')->update(['delivery_status' => 1, "delivery_status_name" => "已发货"], [['order_id', "=", $order_id]]);
            $order_goods_info = model('commander_order_goods')->getInfo([['order_id', '=', $order_id]], 'sku_id,sku_name,sku_image,price,num,order_goods_id');//订单项详情
            //创建待核销记录
