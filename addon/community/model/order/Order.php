...
class Order extends OrderCommon
{
    ...
    public function orderPay($order_info, $pay_type)
    {
        ...
        $result = model("commander_order")->update($data, $condition);
    ...
    public function orderGoodsDelivery($param, $type = 1)
    {
        ...
        try {
            ...
            foreach ($order_goods_id_array as $k => $v) {
                ...
                $order_goods_info = model("commander_order_goods")->getInfo([["order_goods_id", "=", $v], ["site_id", "=", $site_id]], "sku_id,num,order_id,sku_name,sku_image,member_id,refund_status,delivery_status");
                //已退款的订单项不可发货
                ...
                $res = model("commander_order_goods")->update($data, [
                    ['order_goods_id', "=", $v],
                    ['delivery_status', "=", self::DELIVERY_WAIT]
                ]);
        ...
        try {
            ...
            foreach ($order_list as $v) {
                ...
                if ($param['type'] == 'electronicsheet') {//电子面单发货
                    ...
                    $param['delivery_no'] = $result['data']["commander_order"]['LogisticCode'];
                } else {
    ...
    public function orderFileDelivery($param, $site_id)
    {
        ...
        try {
            ...
            for ($i = 2; $i <= $allRow; $i++) {
                ...
                //获取订单信息
                $order_info = model("commander_order")->getInfo([['order_no', '=', $order_no], ['site_id', '=', $site_id]], 'order_id,order_status');
                ...
                if ($delivery_data['type'] == 'electronicsheet') {//电子面单发货
                    ...
                    $delivery_data['delivery_no'] = $result['data']["commander_order"]['LogisticCode'];
                }
    ...
    {
        //统计订单项目
        $count = model("commander_order_goods")->getCount([['order_id', "=", $order_id], ['delivery_status', "=", self::DELIVERY_WAIT], ["refund_status", "<>", 3]], "order_goods_id");
        $delivery_count = model("commander_order_goods")->getCount([['order_id', "=", $order_id], ['delivery_status', "=", self::DELIVERY_DOING], ["refund_status", "<>", 3]], "order_goods_id");
        if ($count == 0 && $delivery_count > 0) {

            $order_info = model("commander_order")->getInfo([['order_id', "=", $order_id]], 'site_id');
...
/*            $message_model = new Message();
    ...
    public function orderAddressUpdate($param, $condition)
    {
        ...
        $order_info = model("commander_order")->getInfo($condition, "order_status");
    ...
    {
        $res = model("commander_order")->getSum($condition, $field);
        return $this->success($res);
    }
}
