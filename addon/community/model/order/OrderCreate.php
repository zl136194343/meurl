...
class OrderCreate extends BaseModel
{
    ...
    public function create($data)
    {
        ...
        try {
            ...
            $data_order = [
                ...
                // 社区自提
                'delivery_community_id'   => $order_item["delivery_community_id"] ?? 0,
                "delivery_community"      => $order_item["delivery_community"] ?? '',
                "delivery_community_info" => $order_item["delivery_community_info"] ?? '',
                "cl_id" => $order_item["cl_id"] ?? 0,
                "cl_name" => $order_item["cl_name"] ?? 0,
            ...
            $order_id = model("commander_order")->add($data_order);
            ...
            foreach ($order_item[ 'goods_list' ] as $k_order_goods => $order_goods) {
                ...
                if(isset($order_goods['activity_type'])) $data_order_goods['activity_type'] = $order_goods['activity_type'];
               
                model("commander_order_goods")->add($data_order_goods);
            ...
            //订单生成后操作
            $result_list = event("CommunityOrderCreate", [ 'order_id' => $order_id ]);
            ...
            //生成整体付费支付单据
            $pay->addPay($calculate_data[ "site_id" ], $out_trade_no, $this->pay_type, $this->order_name, $this->order_name, $this->pay_money, '', 'CommunityOrderPayNotify', '');
    ...
    public function orderType($shop_goods, $data)
    {
        if ($data["is_virtual"] == 1) {
            ...
            } elseif ($shop_goods['delivery']['delivery_type'] == 'community') {
                $order = new CommunityOrder();
                return [
                    'order_type_id'   => 5,
                    'order_type_name' => '社区订单',
                    'order_status'    => $order->order_status[0]
                ];
            }
        }
    ...
    public function calculate($data)
    {
        ...
        foreach ($shop_goods_list[ 'limit_purchase' ] as $item) {
            ...
            switch ($activity_type){
                case 'seckill':
                    // 循环sku商品限购判断
                    foreach ($item['sku'] as $key => $child){
                        $join = [['order o', 'o.order_id = og.order_id', 'left']];
                        $condition = [
                            ['og.sku_id', '=', $key],
                            ['og.activity_type', '=', 'seckill'],
                            ['og.refund_status', '<>', OrderRefund::REFUND_COMPLETE],
                            ['o.order_status', '<>', Order::ORDER_CLOSE],
                        ];
                        $number = model('order_goods')->getSum($condition, 'og.num', 'og', $join);
                        if ($child['activity_max_buy']>0 && $child['num'] + $number > $child['activity_max_buy']) {
                            $this->error = 1;
                            $this->error_msg = "商品“{$item['goods_name']}”秒杀限购{$child['activity_max_buy']}件，您已购买{$number}件";
                        }
                    }
                    break;
                default:
    ...
    public function orderPayment($data)
    {
        ...
        if ($this->is_virtual == 0) {
            foreach ($calculate_data[ 'shop_goods_list' ]['deliver_sort'] as $type) {
                // 社区自提
                if ($type == 'community' && $calculate_data['shop_goods_list']["community_config"]["is_use"] == 1) {
                    //根据坐标查询门店
                    $community_condition = [
                    ...
                    $community_list = $leader_model->getLocationList($community_condition, '*', $latlng);
                    $community_list = $community_list["data"];

                    //如果用户默认选中了门店
                    $cl_id = 0;
                    if ($data[ 'default_cl_id' ] > 0) {
                        if (!empty($community_list)) {
                            $cl_array = array_column($community_list, 'cl_id');
                            if (in_array($data[ 'default_cl_id' ], $cl_array)) {
                                $cl_id = $data[ 'default_cl_id' ];
                            } else {
                                $community_condition = [
                                    ['site_id', '=', $data['site_id']],
                                    ['status', '=', 1],
                                ...
                                $cl_info_result = $leader_model->getLeaderInfo($community_condition, '*');
                                $cl_info = $cl_info_result['data'];
                                if (!empty($cl_info)) {
                                    $cl_id = $data['default_cl_id'];
                                    $community_list[] = $cl_info;
                                }
                            }
                        }
                    }
                    $express_type[] = ["title" => "社区自提", "name" => "community", 'community_list'=>$community_list, 'cl_id'=>$cl_id];
                }
            }
        }
    ...
    public function getCartGoodsList($cart_ids, $member_id, $site_id)
    {
        ...
        ngs.sku_name,ngs.sku_no,ngs.price, ngs.discount_price, ngs.cost_price, ngs.stock, ngs.weight, ngs.volume,
        ngs.sku_image,ngs.site_id,ngs.goods_state,ngs.is_virtual,ngs.is_free_shipping,ngs.shipping_template,
        ngs.goods_class, ngs.goods_class_name, ngs.goods_id,ngs.sku_spec_format,ngs.goods_name,ngs.max_buy,ngs.min_buy
        ';
        $join = [
            ['community_goods_sku ngs', 'ngc.sku_id = ngs.sku_id', 'inner'],
           /* ['site ns', 'ngc.cl_id = ns.cl_id', 'inner']*/
        ];
        
        $goods_list = model("commander_goods_cart")->getList([['cart_id', 'in', $cart_ids], ['member_id', '=', $member_id]], $field, '', 'ngc', $join);
            ...
            foreach ($goods_list as $k => $v) {
                ...
                /*switch ($v['activity_type']) {
                    case 'seckill':
                        ...
                        if(empty($seckill['data'])) return $this->error([], '秒杀商品不存在');
                        $seckill_info = $model->getSeckillInfo($seckill['data']['seckill_id']);
                        if(empty($seckill_info['data'])) return $this->error([], '活动不存在');
                        $price = $seckill['data']['seckill_price'];
                        // 活动限购
                        $v['activity_max_buy'] = $seckill['data']['max_buy'];
                        $v['activity_tag'] = '秒杀';
                        break;
                    default:
                        // 普通或折扣商品
                ...
                if (empty($shop_goods_list)) {
                    $shop_goods_list['site_id']        = 0;
                    $shop_goods_list['site_name']      = '';
                ...
                // 商品sku限购
                if (isset($shop_goods_list['limit_purchase']['goods_'.$v['goods_id']]['sku'][$v['sku_id']])) {
                    $shop_goods_list['limit_purchase']['goods_'.$v['goods_id']]['sku'][$v['sku_id']]['num'] += $v['num'];
                }else{
                    $shop_goods_list['limit_purchase']['goods_'.$v['goods_id']]['sku'][$v['sku_id']]['num'] = $v['num'];
                    if(isset($v['activity_max_buy'])) {
                        $shop_goods_list['limit_purchase']['goods_'.$v['goods_id']]['sku'][$v['sku_id']]['activity_max_buy'] = $v['activity_max_buy'];
                    }
                }
            }
        }
    ...
    public function getShopGoodsList($data)
    {
        ...
        $field = '';
        $sku_info = model("community_goods_sku")->getInfo([ [ 'sku_id', '=', $data[ 'sku_id' ] ] ], $field, 'ngs', '');
    ...
    public function shopOrderCalculate($shop_goods, $data)
    {
        ...
        if ($this->is_virtual == 1) {
            ...
            //查询店铺是否开启社区自提
            $community_config_result = $express_config_model->getCommunityDeliveryConfig($site_id);
            $community_config = $community_config_result["data"];
            
            $shop_goods["community_config"] = $community_config;

            $delivery_array = $data[ 'delivery' ] ?? [];
            
            $delivery_type = $delivery_array[ "delivery_type" ] ?? 'community';
            ...
            if ($delivery_type == "community") {
                // 社区自提
                    ...
                    $delivery_money = 0;
                    $shop_goods['delivery']['delivery_type'] = 'community';
                    if ($shop_goods["community_config"]["is_use"] == 0) {
                        $this->error = 1;
                        $this->error_msg = "社区自提未开启!";
                    }
                    if (empty($data['delivery']["cl_id"])) {
                        $this->error = 1;
                        $this->error_msg = "自提点未选择!";
                        $data['delivery']["cl_id"] = 0;
                    }
                     
                    $status = model('community_leader')->getValue([['cl_id', '=', $data['delivery']["cl_id"]]], 'status');
                    if ($status != 1) {
                        $this->error = 1;
                        $this->error_msg = "自提点未营业!";
                    }
                    
                    $shop_goods['delivery']['cl_id'] = $data['delivery']["cl_id"] ?? 0;
                    
                    $shop_goods = $this->communityOrderData($shop_goods, $data);
    ...
    public function addOrderCronClose($order_id, $site_id)
    {
        ...
        if ($order_config[ "value" ][ "auto_close" ] > 0) {
            ...
            $cron_model = new Cron();
            $cron_model->addCron(1, 0, "订单自动关闭", "CronCommanderOrderClose", $execute_time, $order_id);
        }
    }
...
//        if ($result["code"] >= 0) {
        ...
        if ($balance > 0) {
            ...
            $result = $member_account_model->addMemberAccountT($site_id, $data[ "member_id" ], "balance", -$real_balance, "commander_order", $data['order_id'], "订单余额抵扣,扣除不可提现余额:" . $real_balance);
        ...
        //                }
        //                $result = $member_account_model->addMemberAccount($data["member_id"], "balance", -$real_balance, "commander_order", "余额抵扣","订单余额抵扣,扣除不可提现余额:".$real_balance);
        //            }
        
        if ($surplus_banance > 0) {
            
            $result = $member_account_model->addMemberAccountT($site_id, $data[ "member_id" ], "balance_money", -$surplus_banance, "commander_order", $data['order_id'], "订单余额抵扣,扣除可提现余额:" . $surplus_banance);
...
//        }
     ...
     * @return array
     */
    public function communityOrderData($shop_goods, $data)
    {
        
        $temp_data = [];
        $delivery_cl_id = $shop_goods['delivery']['cl_id'] ?? 0; // 团长id

        if ($delivery_cl_id > 0) {
            $condition = [
                ["cl_id", "=", $delivery_cl_id],
                ["site_id", "=", $shop_goods['site_id']],
                ["status", "=", 1],
            ];
            $cl_info = model('community_leader')->getInfo($condition, 'cl_id,name,community,address,full_address,longitude,latitude,name,mobile,community_img');
            if (empty($cl_info)) {
                $this->error = 1;
                $this->error_msg = "当前社区不存在或未营业!";
            } else {
                $temp_data[ "delivery_community_id" ] = $delivery_cl_id;
                $temp_data[ "delivery_community" ] = $cl_info["community"];
                $temp_data[ "cl_id" ] = $cl_info["cl_id"];
                $temp_data[ "cl_name" ] = $cl_info["name"];
                $delivery_cl_info = [
                    "full_address" => $cl_info["full_address"] . $cl_info["address"],
                    "longitude"    => $cl_info["longitude"],
                    "latitude"     => $cl_info["latitude"],
                    "name"         => $cl_info["name"],
                    "mobile"       => $cl_info["mobile"],
                    "store_image"  => $cl_info["community_img"],
                ];
                $temp_data["delivery_community_info"] = json_encode($delivery_cl_info, JSON_UNESCAPED_UNICODE);
            }
        } else {
            $this->error = 1;
            $this->error_msg = "团长ID不可为空!";
        }
        return array_merge($shop_goods, $temp_data);
    }
}
