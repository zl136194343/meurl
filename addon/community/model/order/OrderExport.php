...
class OrderExport extends BaseModel
{
    ...
    public function orderExport($condition, $condition_desc, $site_id = 0, $join = [])
    {
        ...
        try {
            ...
            if (dir_mkdir($file_path)) {
                ...
                $order_table = Db::name('commander_order')->where($condition)->alias($alias);

                if(!empty($join)){
                    $join = [
                        [
                            'commander_order_goods og',
    ...
    public function orderGoodsExport($condition, $condition_desc, $site_id = 0)
    {
        ...
        try {
            ...
            $order_table = Db::name('commander_order_goods')->where($condition)->alias($alias);
    ...
    {
        $res = model("commander_order_export")->add($data);
    ...
    {
        $res = model("commander_order_export")->update($data, $condition);
        return $this->success($res);
    }
    ...
    {
        //先查询数据
        $list = model("commander_order_export")->getList($condition, '*');
        if(!empty($list)){
            foreach($list as $k => $v){
            ...
            $res = model("commander_order_export")->delete($condition);
        }
    ...
    {
        $list = model("commander_order_export")->getList($condition, $field, $order);
    ...
    {
        $list = model('commander_order_export')->pageList($condition, $field, $order, $page, $page_size);
        return $this->success($list);
    }
    ...
    {
        $res = model("commander_order_refund_export")->add($data);
        return $this->success($res);
    }
    ...
    {
        $res = model("commander_order_refund_export")->update($data, $condition);
        return $this->success($res);
    }
    ...
    {
        $res = model("commander_order_refund_export")->delete($condition);
        return $this->success($res);
    }
    ...
    public function getRefundExport($condition, $field = "*", $order = '')
    {
        ...
        $list = model('commander_order_refund_export')->getList($condition, $field, $order);
        return $this->success($list);
    }
