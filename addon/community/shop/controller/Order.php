...
use addon\community\model\order\OrderExport;
use phpoffice\phpexcel\Classes\PHPExcel;
use phpoffice\phpexcel\Classes\PHPExcel\Writer\Excel2007;
use think\facade\Config;
use addon\community\model\system\Promotion as PromotionModel;
...
//                $condition[] = [ 'order_status', 'in', array_keys($order_status_list) ];
            ...
            if ($cl_name != "") {
                $condition[] = [ 'a.cl_name', 'like', "%$cl_name%" ];
            }
            $join[] = ['community_delivery cd', 'cd.delivery_id = a.delivery_id', 'left'];
            $list = $order_common_model->getOrderPageList($condition, $page_index, $page_size, "a.create_time desc", $field = 'a.*,cd.delivery_no', $alias, $join);
            ...
            $order_model = new CommunityOrder();
            $order_status_list = $order_model->order_status;
            $this->assign("order_status_list", $order_type_list[5]['status']);//订单状态
    ...
    public function detail()
    {
        ...
        switch ( $order_detail[ "order_type" ] ) {
            ...
            case 4 :
                ...
                break;
            case 5 :
                $template = "communityorder/detail";
                break;
        }
    ...
    public function config()
    {
        if (request()->isAjax()) {
            ...
            $trade_config = [
                'book_time' => input('book_time', 0),// 发货下单时间点
                'pickup_time' => input('pickup_time', 0),// 自提时间点
                'delivery_type' => input('delivery_type', 1),// 发货类型 1当日达 2次日达 3隔日达
            ];
            ...
            $config_model->setTradeConfig($trade_config, $this->site_id, $this->app_module);

            $delivery_order_model = new DeliveryOrderModel();
            $delivery_order_model->resetCreateCommunityDeliveryOrderCronExecuteTime($this->site_id);
            return $res;
        } else {
            $config_model = new ConfigModel();
            //订单事件时间设置
            ...
            //社区订单设置
            $trade_config = $config_model->getTradeConfig($this->site_id, $this->app_module);
            ...
            $this->assign('trade', $trade_config['data']['value']);
...
/*    public function delivery()
    {
        if (request()->isAjax()) {
            ...
            $order_goods_ids = model('commander_order_goods')->getList([['order_id','=',input("order_id", 0)]],'order_goods_id');
            $order_goods_ids = implode(',',array_column($order_goods_ids,'order_goods_id'));
            $data = array (
    ...
    public function delivery(){
        ...
        $order_model = new OrderModel();
        return $order_model ->get_delivery($order_id);
