...
class Delivery extends BaseShop
{
    ...
    public function express()
    {
        ...
        // 社区自提
        $config_result = $config_model->getCommunityDeliveryConfig($this->site_id);
        $community_delivery_config = $config_result[ 'data' ];
        $this->assign('community_delivery_config', $community_delivery_config);
        // 发货类型排序
     ...
     * @return array
     */
    public function modifyCommunityStatus()
    {
        $config_model = new ConfigModel();
        if (request()->isAjax()) {
            $is_use = input('is_use', 0);
            $data = array ();
            $result = $config_model->setCommunityDeliveryConfig($data, $is_use, $this->site_id);
            return $result;
        }
    }
}