...
class OrderRefundFinish
{
    ...
    public function handle($data)
    {
        ...
        $order_info_result = $order_model->getOrderInfo([['order_id', '=', $data['order_id']], ['order_type', 'in', [2, 4, 5]]]);
        ...
        if (!empty($order_info)) {
            ...
            } else if ($order_info['order_type'] == "5") {//社区订单
                $verify_code = $order_info['virtual_code'];
            }

            $verify_model       = new Verify();
            $verify_condition   = [['verify_code', '=', $verify_code]];
            ...
            if (!empty($verify_info)) {
                ...
                $verify_data   = ['verify_content_json' => $json_string];
