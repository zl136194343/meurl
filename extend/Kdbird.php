...
class Kdbird
{
    ...
    public function __construct($config){
        ...
        if(empty($config['is_pay']) || $config['is_pay'] == 0){
            $this->RequestType = '1002';
        }else{
            $this->RequestType = '8001';
        }
    }

    public function orderTracesSubByJson($shipper_code, $logistic_code, $customer_name = ''){
        $request_array = array(
        ...
        if($shipper_code == 'SF' ){
            $request_array['CustomerName'] = $customer_name;
        }
