<?php /*a:1:{s:42:"./app/component/view/image_ads/design.html";i:1614516072;}*/ ?>
<nc-component v-bind:data="data[index]" class="image-ads">
					...
					<div class="layui-input-block">
						...
						<ul class="ns-icon">
							<li :class="{'ns-text-color ns-border-color ns-bg-color-diaphaneity': nc.selectedTemplate == 'single-graph'}" @click="nc.selectedTemplate = 'single-graph'">
								<img v-if="nc.selectedTemplate == 'single-graph'" src="<?php echo htmlentities($resource_path); ?>/image_ads/img/single_1<?php echo $app_module=='shop' ? '' : '_admin'; ?>.png" />
								<img v-else src="<?php echo htmlentities($resource_path); ?>/image_ads/img/single.png" />
							</li>
							<li :class="{'ns-text-color ns-border-color ns-bg-color-diaphaneity': nc.selectedTemplate == 'carousel-posters'}" @click="nc.selectedTemplate = 'carousel-posters'">
								<img v-if="nc.selectedTemplate == 'carousel-posters'" src="<?php echo htmlentities($resource_path); ?>/image_ads/img/banner_1<?php echo $app_module=='shop' ? '' : '_admin'; ?>.png" />
								<img v-else src="<?php echo htmlentities($resource_path); ?>/image_ads/img/banner.png" />
							</li>
							<li :class="{'ns-text-color ns-border-color ns-bg-color-diaphaneity': nc.selectedTemplate == 'horizontal-sliding'}" @click="nc.selectedTemplate = 'horizontal-sliding'">
								<img v-if="nc.selectedTemplate == 'horizontal-sliding'" src="<?php echo htmlentities($resource_path); ?>/image_ads/img/cross_slip_1<?php echo $app_module=='shop' ? '' : '_admin'; ?>.png" />
								<img v-else src="<?php echo htmlentities($resource_path); ?>/image_ads/img/cross_slip.png" />
							</li>
						</ul>
						<!-- <div class="selected-template-list">
							<ul>
								<li v-bind:class="[nc.selectedTemplate == 'single-graph' ? 'ns-border-color ns-text-color' : '']"
									...
									<img src="<?php echo htmlentities($resource_path); ?>/image_ads/img/single_graph.png">
									<div>单图</div>
								</li>
	...
	<template slot="resource">
		<js>
			var imageAdsResourcePath = "<?php echo htmlentities($resource_path); ?>";
		</js>
