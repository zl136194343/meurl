var vue = new Vue({
	...
	methods: {
				...
				layer.open({
					...
					success: function (layero, index) {
						...
						$parent.find(".del").click(function () {
							...
							$parent.find(".upload-img-box").html(`<div class="ns-upload-default"><img src="${ns_url.IMGPATH}/upload_img.png" /></div>`);
					...
					yes: function (index, layero) {
						...
						if (tag == 'add') {
							...
							self.data.menuList[i].link = Object.keys(self.tempLink).length ? self.tempLink : self.data.menuList[i].link;
		...
		save: function () {
			...
			$.ajax({
				type: 'POST',
				url: ns.url("admin/diy/memberindex"),
				data: {data: data},
				dataType: 'JSON',
