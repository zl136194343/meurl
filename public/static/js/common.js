 ...
 */
ns.select_link = function (link, support_diy_view, callback) {
	if (post == "shop") support_diy_view = 'DIY_VIEW_SHOP_INDEX';
	var url = ns.url("admin/commonutil/link");
	if (show_link_box_flag) {
		show_link_box_flag = false;
		$.post(url, {link: JSON.stringify(link), support_diy_view: support_diy_view,post:post}, function (str) {
...
ns.sizeformat = function (limit) {
	...
	limit = parseFloat(limit.substring(0, (limit.length - 2))); //得到纯数字
...
function Table(options) {
	...
	if (options.page == undefined) {
	    options.page = {
	        layout: ['count', 'limit', 'prev', 'page', 'next'],
			// limit: $.cookie('pageSize'+site_id) || 10,
			// curr: $.cookie('currPage'+site_id) || 1
			limit: 10,
			curr: 1
	    };
	}
	...
	options.done = function (res, curr, count) {
		//加载图片放大
		loadImgMagnify();
		// $.cookie('currPage'+site_id, curr, {path: window.location.pathname});
		// $.cookie('pageSize'+site_id, $('.layui-laypage-limits select[lay-ignore]').val(), {path: window.location.pathname});
...
function Upload(options) {
    ...
    if (options.post == 'supply') options.post += '://supply';
            ...
            if (res.code >= 0) {
                ...
                if (options.accept == 'images') {
					if(elemChildImg.length){
						var tempId =  $(options.elem).children('.preview_img').attr('id')?$(options.elem).children('.preview_img').attr('id'):'preview_'+$(options.elem).attr('id');
